...
public abstract class AbstractPaginateList
{
     ...
     * @param defaultSort THe default sort column (the rest attribute) that later will be mapped to an internal engine
     *            name
     */
        ...
        String defaultSort, Map<String, QueryProperty> properties)
    {
        ...
        if (sort != null && !properties.isEmpty())
        {
            ...
            if (qp == null)
            {
                throw new ActivitiIllegalArgumentException("Value for param 'sort' is not valid, '" + sort
            ...
            {
                throw new ActivitiIllegalArgumentException("Value for param 'order' is not valid : '" + order
                    + "', must be 'asc' or 'desc'");
            }
        }
     ...
     */
    @SuppressWarnings("rawtypes")
    public DataResponse paginateList(Map<String, String> requestParams, Query query, String defaultSort,
