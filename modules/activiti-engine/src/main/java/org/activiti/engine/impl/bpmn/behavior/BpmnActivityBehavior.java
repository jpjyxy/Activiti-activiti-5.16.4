...
public class BpmnActivityBehavior implements Serializable
{
    ...
    /**
     * Performs the default outgoing BPMN 2.0 behavior, which is having parallel paths of executions for the outgoing
     * sequence flow.
     * 
     * More precisely: every sequence flow that has a condition which evaluates to true (or which doesn't have a
     * condition), is selected for continuation of the process instance. If multiple sequencer flow are selected,
    ...
    /**
     * Performs the default outgoing BPMN 2.0 behavior (@see {@link #performDefaultOutgoingBehavior(ActivityExecution)}
     * ), but without checking the conditions on the outgoing sequence flow.
     * 
     * This means that every outgoing sequence flow is selected for continuing the process instance, regardless of
     * having a condition or not. In case of multiple outgoing sequence flow, multiple parallel paths of executions will
     * be created.
     */
     ...
     * @param throwExceptionIfExecutionStuck If true, an {@link ActivitiException} will be thrown in case no transition
        ...
        boolean throwExceptionIfExecutionStuck, List<ActivityExecution> reusableExecutions)
    {
        ...
        else
        {
            ...
            if (defaultSequenceFlow != null)
            {
                ...
                {
                    throw new ActivitiException("Default sequence flow '" + defaultSequenceFlow
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (log.isDebugEnabled())
                    {
                        log.debug("No outgoing sequence flow found for {}. Ending execution.", execution.getActivity()
                            .getId());
                    }
                    execution.end();
