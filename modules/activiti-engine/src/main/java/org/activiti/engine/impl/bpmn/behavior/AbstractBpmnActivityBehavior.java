...
public class AbstractBpmnActivityBehavior extends FlowNodeActivityBehavior
{
     ...
     * {@link FlowNodeActivityBehavior#leave(ActivityExecution)} is called. This way, we can check if the activity has
     * loop characteristics, and delegate to the behavior if this is the case.
     */
    ...
    protected void createCompensateEventSubscription(ActivityExecution execution)
    {
        String compensationHandlerId =
            (String)execution.getActivity().getProperty(BpmnParse.PROPERTYNAME_COMPENSATION_HANDLER_ID);
        ...
        CompensateEventSubscriptionEntity compensateEventSubscriptionEntity =
            CompensateEventSubscriptionEntity.createAndInsert(scopeExecution);
