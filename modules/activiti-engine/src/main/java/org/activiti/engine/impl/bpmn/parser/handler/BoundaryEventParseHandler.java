...
public class BoundaryEventParseHandler extends AbstractFlowNodeBpmnParseHandler<BoundaryEvent>
{
    ...
    protected void executeParse(BpmnParse bpmnParse, BoundaryEvent boundaryEvent)
    {
        ...
        {
            logger.warn("Invalid reference in boundary event. Make sure that the referenced activity is defined in the same scope as the boundary event "
                + boundaryEvent.getId());
            return;
        }
        
        ActivityImpl nestedActivity =
            createActivityOnScope(bpmnParse, boundaryEvent, BpmnXMLConstants.ELEMENT_EVENT_BOUNDARY, parentActivity);
        ...
        if (eventDefinition instanceof TimerEventDefinition
            ...
            || eventDefinition instanceof SignalEventDefinition || eventDefinition instanceof CancelEventDefinition
