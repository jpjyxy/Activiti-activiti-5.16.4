...
/**
 * Default implementation of the {@link ListenerFactory}. Used when no custom {@link ListenerFactory} is injected on the
...
public class DefaultListenerFactory extends AbstractBehaviorFactory implements ListenerFactory
{
    ...
    {
        return new ClassDelegate(activitiListener.getImplementation(),
    ...
    protected Class<?> getEntityType(String entityType)
    {
        if (entityType != null)
        {
            ...
            if (entityClass == null)
            {
                throw new ActivitiIllegalArgumentException("Unsupported entity-type for an ActivitiEventListener: "
                    + entityType);
            }
            return entityClass;
        }
