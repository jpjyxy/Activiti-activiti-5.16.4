...
public interface TaskService
{
    ...
    /**
     * Saves the given task to the persistent data store. If the task is already present in the persistent store, it is
     * updated. After a new task has been saved, the task instance passed into this method is updated with the id of the
     * newly created task.
     * 
     ...
     * 
     * @param taskId The id of the task that will be deleted, cannot be null. If no task exists with the given taskId,
     *            the operation is ignored.
     * @throws ActivitiObjectNotFoundException when the task with given id does not exist.
     * @throws ActivitiException when an error occurs while deleting the task or in case the task is part of a running
     *             process.
     */
     ...
     * 
     * @param taskIds The id's of the tasks that will be deleted, cannot be null. All id's in the list that don't have
     *            an existing task will be ignored.
     * @throws ActivitiObjectNotFoundException when one of the task does not exist.
     * @throws ActivitiException when an error occurs while deleting the tasks or in case one of the tasks is part of a
     *             running process.
     */
    ...
    /**
     * Claim responsibility for a task: the given user is made assignee for the task. The difference with
     * {@link #setAssignee(String, String)} is that here a check is done if the task already has a user assigned to it.
    ...
    /**
     * Delegates the task to another user. This means that the assignee is set and the delegation state is set to
     * {@link DelegationState#PENDING}. If no owner is set on the task, the owner is set to the current assignee of the
     * task.
     * 
    ...
    /**
     * Marks that the assignee is done with this task and that it can be send back to the owner. Can only be called when
     * this task is {@link DelegationState#PENDING} delegation. After this method returns, the
     * {@link Task#getDelegationState() delegationState} is set to {@link DelegationState#RESOLVED}.
     * 
    ...
    /**
     * Marks that the assignee is done with this task providing the required variables and that it can be sent back to
     * the owner. Can only be called when this task is {@link DelegationState#PENDING} delegation. After this method
     ...
     * @param taskId the id of the task to complete, cannot be null.
     * @param variables task parameters. May be null or empty.
     * @param localScope If true, the provided variables will be stored task-local, instead of process instance wide
     *            (which is the default for {@link #complete(String, Map)}).
     * @throws ActivitiObjectNotFoundException when no task exists with the given id.
     */
    void complete(String taskId, Map<String, Object> variables, boolean localScope);
    
    /**
     * Changes the assignee of the given task to the given userId. No check is done whether the user is known by the
     * identity component.
     * 
    ...
    /**
     * Transfers ownership of this task to another user. No check is done whether the user is known by the identity
     * component.
     * 
     * @param taskId id of the task, cannot be null.
    ...
    /**
     * Retrieves the {@link IdentityLink}s associated with the given task. Such an {@link IdentityLink} informs how a
     * certain identity (eg. group or user) is associated with a certain task (eg. as candidate, assignee, etc.)
     */
