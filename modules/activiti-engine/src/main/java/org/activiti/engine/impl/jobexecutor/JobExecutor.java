 ...
 * 
 * <p>
 * You should generally only have one of these per Activiti instance (process engine) in a JVM. In clustered situations,
 * you can have multiple of these running against the same queue + pending job list.
 * </p>
 * 
 * @author Daniel Meyer
 * @author Joram Barrez
...
public abstract class JobExecutor
{
    ...
    /**
     * To avoid deadlocks, the default for this is one. This way, in a clustered setup, multiple job executors can
     * acquire jobs without creating a deadlock due to fetching multiple jobs at once and trying to lock them all at
     * once.
     * 
    ...
    /**
     * Called when a new job was added by the process engine to which this job executor belongs. This is a hint, that
     * for example the acquiring needs to start again when it would be sleeping.
     */
