...
public class JobRetryCmd implements Command<Object>
{
    ...
    public Object execute(CommandContext commandContext)
    {
        ...
        {
            log.error("activitiy or FailedJobRetryTimerCycleValue is null in job " + jobId
            ...
            if (job.getDuedate() == null)
            {
                ...
                job.setDuedate(calculateDueDate(commandContext, commandContext.getProcessEngineConfiguration()
                    .getAsyncFailedJobWaitTime(), null));
            }
            else
            {
                // add default wait time for failed job
                job.setDuedate(calculateDueDate(commandContext, commandContext.getProcessEngineConfiguration()
                    .getDefaultFailedJobWaitTime(), job.getDuedate()));
            }
        ...
        else
        {
            ...
            try
            {
                DurationHelper durationHelper =
                    new DurationHelper(failedJobRetryTimeCycle, commandContext.getProcessEngineConfiguration()
                        .getClock());
        ...
        if (eventDispatcher.isEnabled())
        {
            ...
            eventDispatcher.dispatchEvent(ActivitiEventBuilder.createEntityEvent(ActivitiEventType.JOB_RETRIES_DECREMENTED,
                job));
        }
    ...
    private ActivityImpl getCurrentActivity(CommandContext commandContext, JobEntity job)
    {
        ...
        else if (TimerStartEventJobHandler.TYPE.equals(type))
        {
            ...
            ProcessDefinitionEntity processDefinition =
                deploymentManager.findDeployedLatestProcessDefinitionByKeyAndTenantId(job.getJobHandlerConfiguration(),
                    job.getTenantId());
