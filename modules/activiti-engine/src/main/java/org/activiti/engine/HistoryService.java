...
/**
 * Service exposing information about ongoing and past process instances. This is different from the runtime information
 * in the sense that this runtime information only contains the actual runtime state at any given moment and it is
 * optimized for runtime process execution performance. The history information is optimized for easy querying and
 * remains permanent in the persistent storage.
 * 
...
public interface HistoryService
{
    ...
    /**
     * Deletes historic task instance. This might be useful for tasks that are {@link TaskService#newTask() dynamically
     * created} and then {@link TaskService#complete(String) completed}. If the historic task instance doesn't exist, no
     * exception is thrown and the method returns normal.
     */
    void deleteHistoricTaskInstance(String taskId);
    
    /**
     * Deletes historic process instance. All historic activities, historic task and historic details (variable updates,
    ...
    /**
     * Retrieves the {@link HistoricIdentityLink}s associated with the given task. Such an {@link IdentityLink} informs
     * how a certain identity (eg. group or user) is associated with a certain task (eg. as candidate, assignee, etc.),
     * even if the task is completed as opposed to {@link IdentityLink}s which only exist for active tasks.
     */
     ...
     * {@link IdentityLink} informs how a certain identity (eg. group or user) is associated with a certain process
     * instance, even if the instance is completed as opposed to {@link IdentityLink}s which only exist for active
     * instances.
     */
