...
public interface HistoricDetailQuery extends Query<HistoricDetailQuery, HistoricDetail>
{
    ...
    /**
     * Only select historic variable updates with the given process instance. {@link ProcessInstance) ids and
     * 
    ...
    /**
     * Only select historic variable updates with the given execution. Note that {@link Execution} ids are not stored in
     * the history as first class citizen, only process instances are.
     */
    ...
    /**
     * Exclude all task-related {@link HistoricDetail}s, so only items which have no task-id set will be selected. When
     * used togheter with {@link #taskId(String)}, this call is ignored task details are NOT excluded.
     */
