...
public class ExecutionEntity extends VariableScopeImpl implements ActivityExecution, ExecutionListenerExecution,
    Execution, PvmExecution, ProcessInstance, InterpretableExecution, PersistentObject, HasRevision
{
    ...
    /**
     * indicates if this execution represents an active path of execution. Executions are made inactive in the following
     * situations:
     * <ul>
    ...
    /**
     * when execution structure is pruned during a takeAll, then the original execution has to be resolved to the
     * replaced execution.
     * 
    ...
    /**
     * next operation. process execution is in fact runtime interpretation of the process model. each operation is a
     * logical unit of interpretation of the process. so sequentially processing the operations drives the
    ...
    public void initialize()
    {
        ...
        List<EventSubscriptionDeclaration> eventSubscriptionDeclarations =
            (List<EventSubscriptionDeclaration>)scope.getProperty(BpmnParse.PROPERTYNAME_EVENT_SUBSCRIPTION_DECLARATION);
    ...
    /**
     * removes an execution. if there are nested executions, those will be ended recursively. if there is a parent, this
     * method removes the bidirectional relation between parent and this execution.
     */
    public void end()
    {
        isActive = false;
    ...
    public void signal(String signalName, Object signalData)
    {
        ...
        try
        {
                ...
                && Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled())
            {
                Context.getProcessEngineConfiguration()
                    .getEventDispatcher()
                    .dispatchEvent(ActivitiEventBuilder.createSignalEvent(ActivitiEventType.ACTIVITY_SIGNALED,
                        signalledActivityId,
                        signalName,
                        signalData,
                        this.id,
    ...
    /**
     * @param fireActivityCompletionEvent This method can be called from other places (like {@link #takeAll(List, List)}
     *            ), where the event is already fired. In that case, false is passed an no second event is fired.
     */
    ...
    public void takeAll(List<PvmTransition> transitions, List<ActivityExecution> recyclableExecutions)
    {
        ...
        recyclableExecutions =
            (recyclableExecutions != null ? new ArrayList<ActivityExecution>(recyclableExecutions)
    ...
    protected void ensureProcessInstanceInitialized()
    {
        ...
        {
            processInstance =
                Context.getCommandContext().getExecutionEntityManager().findExecutionById(processInstanceId);
        }
    }
    ...
    protected void ensureSuperExecutionInitialized()
    {
        ...
        {
            superExecution =
                Context.getCommandContext().getExecutionEntityManager().findExecutionById(superExecutionId);
        }
    }
    ...
    protected void ensureSubProcessInstanceInitialized()
    {
        ...
        {
            subProcessInstance =
                Context.getCommandContext().getExecutionEntityManager().findSubProcessInstanceBySuperExecutionId(id);
        }
    }
    ...
    public void setReplacedBy(InterpretableExecution replacedBy)
    {
        ...
        for (TaskEntity task : allTasks)
        {
            ...
            List<VariableInstanceEntity> variables =
                (List)commandContext.getVariableInstanceEntityManager().findVariableInstancesByTaskId(task.getId());
        ...
        List<VariableInstanceEntity> variables =
            (List)commandContext.getVariableInstanceEntityManager().findVariableInstancesByExecutionId(id);
        ...
        ExecutionEntity sourceActivityExecution)
    {
            ...
            && Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled())
        {
            Context.getProcessEngineConfiguration()
                .getEventDispatcher()
                .dispatchEvent(ActivitiEventBuilder.createVariableEvent(ActivitiEventType.VARIABLE_CREATED,
                    variableName,
                    value,
                    result.getType(),
                    result.getTaskId(),
        ...
        ExecutionEntity sourceActivityExecution)
    {
            ...
            && Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled())
        {
            Context.getProcessEngineConfiguration()
                .getEventDispatcher()
                .dispatchEvent(ActivitiEventBuilder.createVariableEvent(ActivitiEventType.VARIABLE_UPDATED,
    ...
    /**
     * Adds an IdentityLink for this user with the specified type, but only if the user is not associated with this
     * instance yet.
     **/
    ...
    public int getCachedEntityState()
    {
        ...
        // Only mark a flag as false when the list is not-null and empty. If null, we can't be sure there are no entries
        // in it since
        ...
        cachedEntityState =
            BitMaskUtil.setBit(cachedEntityState,
