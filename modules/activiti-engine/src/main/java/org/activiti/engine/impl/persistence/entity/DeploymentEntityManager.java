...
public class DeploymentEntityManager extends AbstractManager
{
    ...
    public void deleteDeployment(String deploymentId, boolean cascade)
    {
        ...
        for (ProcessDefinition processDefinition : processDefinitions)
        {
            ...
            if (timerStartJobs != null && !timerStartJobs.isEmpty())
            {
                ...
                long nrOfVersions =
                    new ProcessDefinitionQueryImpl(Context.getCommandContext()).processDefinitionKey(processDefinition.getKey())
                        .count();
                    ...
                    for (Job job : timerStartJobs)
                    {
                        if (Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled())
                        {
                            Context.getProcessEngineConfiguration()
                                .getEventDispatcher()
                                .dispatchEvent(ActivitiEventBuilder.createEntityEvent(ActivitiEventType.JOB_CANCELED,
                                    job,
                                    null,
                                    null,
            ...
            List<EventSubscriptionEntity> findEventSubscriptionsByConfiguration =
                Context.getCommandContext()
    ...
    @SuppressWarnings("unchecked")
    public List<Deployment> findDeploymentsByNativeQuery(Map<String, Object> parameterMap, int firstResult,
        int maxResults)
    {
