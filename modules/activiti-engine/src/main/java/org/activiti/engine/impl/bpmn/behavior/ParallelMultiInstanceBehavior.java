...
public class ParallelMultiInstanceBehavior extends MultiInstanceActivityBehavior
{
    ...
    /**
     * Handles the parallel case of spawning the instances. Will create child executions accordingly for every instance
     * needed.
     */
     ...
     * {@link AbstractBpmnActivityBehavior#leave(ActivityExecution)} method. Handles the completion of one of the
     * parallel instances
     */
    public void leave(ActivityExecution execution)
    {
        ...
        if (joinedExecutions.size() >= nrOfInstances || completionConditionSatisfied(execution))
        {
            ...
            for (ExecutionEntity executionToRemove : executionsToRemove)
            {
                ...
                {
                    LOGGER.debug("Execution {} still active, but multi-instance is completed. Removing this execution.",
                        executionToRemove);
                }
