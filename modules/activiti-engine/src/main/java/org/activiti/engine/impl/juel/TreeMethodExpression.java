...
/**
 * A method expression is ready to be evaluated (by calling either {@link #invoke(ELContext, Object[])} or
 * {@link #getMethodInfo(ELContext)}).
 * 
...
public final class TreeMethodExpression extends org.activiti.engine.impl.javax.el.MethodExpression
{
    ...
    /**
     * Create a new method expression. The expression must be an lvalue expression or literal text. The expected return
     * type may be <code>null</code>, meaning "don't care". If it is an lvalue expression, the parameter types must not
     * be <code>null</code>. If it is literal text, the expected return type must not be <code>void</code>.
     * 
     ...
     */
    public TreeMethodExpression(TreeStore store, FunctionMapper functions, VariableMapper variables,
        TypeConverter converter, String expr, Class<?> returnType, Class<?>[] paramTypes)
    {
        super();
     ...
     * versa. Two method expressions are equal if
     * <ol>
