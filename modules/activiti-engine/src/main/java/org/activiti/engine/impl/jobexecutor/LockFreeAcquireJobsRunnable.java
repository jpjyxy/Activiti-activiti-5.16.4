...
/**
 * {@link AcquireJobsRunnable} implementation with sole purpose of 'acquiring' jobs, which means setting the current job
 * executor name into the lock owner colum to 'acquire' it.
 * 
...
public class LockFreeAcquireJobsRunnable implements AcquireJobsRunnable
{
        ...
        while (!isInterrupted)
        {
            ...
            try
            {
                ...
                nrOfAquiredJobs = commandExecutor.execute(new Command<Integer>()
                {
                    ...
                    {
                        return commandContext.getJobEntityManager().updateJobLockForAllJobs(jobExecutor.getLockOwner(),
                    ...
                    protected Date getLockExpirationTime(CommandContext commandContext, int lockTimeInMillis)
                    {
                        ...
                        gregorianCalendar.setTime(commandContext.getProcessEngineConfiguration()
                            .getClock()
                            .getCurrentTime());
    ...
    protected void putAcquiredJobsOnQueue()
    {
        ...
        boolean refetchJobs = false; // Optimization: when doing the refetch at the end of the while, avoid doing it to
                                     // much by keeping this boolean
        while (!jobs.isEmpty())
        {
            ...
            for (JobEntity job : jobs)
            {
                ...
                {
                    // If the queue is full, the rejection handler will execute it in the current thread (ie: this
                    // thread)
    ...
    protected List<JobEntity> fetchJobs(final int start)
    {
        ...
        return jobExecutor.getCommandExecutor().execute(new Command<List<JobEntity>>()
        {
            ...
            {
                return commandContext.getJobEntityManager().findJobsByLockOwner(jobExecutor.getLockOwner(),
                    start,
                    jobExecutor.getJobFetchBatchSize());
            }
        });
    }
