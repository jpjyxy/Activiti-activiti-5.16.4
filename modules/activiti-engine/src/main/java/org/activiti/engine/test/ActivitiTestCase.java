 ...
 * 
 * <p>
 * The ProcessEngine and the services available to subclasses through protected member fields. The processEngine will be
 * initialized by default with the activiti.cfg.xml resource on the classpath. To specify a different configuration
 * file, override the {@link #getConfigurationResource()} method. Process engines will be cached statically. The first
 * time the setUp is called for a given configuration resource, the process engine will be constructed.
 * </p>
 * 
 * <p>
 * You can declare a deployment with the {@link Deployment} annotation. This base class will make sure that this
 * deployment gets deployed in the setUp and {@link RepositoryService#deleteDeploymentCascade(String, boolean) cascade
 * deleted} in the tearDown.
 * </p>
 * 
 * <p>
 * This class also lets you {@link #setCurrentTime(Date) set the current time used by the process engine}. This can be
 * handy to control the exact time that is used by the engine in order to verify e.g. e.g. due dates of timers. Or
 * start, end and duration times in the history service. In the tearDown, the internal clock will automatically be reset
 * to use the current system time rather then the time that was set during a test method.
 * </p>
 * 
 * @author Tom Baeyens
 */
