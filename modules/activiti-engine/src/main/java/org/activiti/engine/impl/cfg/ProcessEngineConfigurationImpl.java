...
public abstract class ProcessEngineConfigurationImpl extends ProcessEngineConfiguration
{
    ...
    /**
     * the configurable list which will be {@link #initInterceptorChain(java.util.List) processed} to build the
     ...
     * 
     * Unfortunately, this feature is not available on some platforms (JDK 6, JBoss), hence the reason why it is
     * disabled by default. If your platform allows the use of StaxSource during XML parsing, do enable it.
     */
    protected boolean enableSafeBpmnXml = false;
    
    /**
     * The following settings will determine the amount of entities loaded at once when the engine needs to load
     * multiple entities (eg. when suspending a process definition with all its process instances).
     * 
     * The default setting is quite low, as not to surprise anyone with sudden memory spikes. Change it to something
     * higher if the environment Activiti runs in allows it.
     */
     ...
     * 
     * If set to true, in the {@link DbSqlSession} during the handling of delete operations, those operations of the
     * same type are merged together. (eg if you have two 'DELETE from X where id=Y' and 'DELETE from X where id=W', it
     * will be merged into one delete statement 'DELETE from X where id=Y or id=W'.
     */
    ...
    public void initDatabaseType()
    {
        ...
        try
        {
            ...
            if (databaseType == null)
            {
                throw new ActivitiException("couldn't deduct database type from database product name '"
                    + databaseProductName + "'");
            }
            ...
            try
            {
                ...
                if (databaseType != null)
                {
                    ...
                    properties.put("limitBeforeNativeQuery",
                        ObjectUtils.toString(DbSqlSessionFactory.databaseSpecificLimitBeforeNativeQueryStatements.get(databaseType)));
                }
    ...
    protected void initJpa()
    {
        ...
        {
            sessionFactories.put(EntityManagerSession.class, new EntityManagerSessionFactory(jpaEntityManagerFactory,
