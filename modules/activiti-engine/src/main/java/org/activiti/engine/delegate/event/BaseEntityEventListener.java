...
/**
 * Base event listener that can be used when implementing an {@link ActivitiEventListener} to get notified when an
 * entity is created, updated, deleted or if another entity-related event occurs.
 * 
...
public class BaseEntityEventListener implements ActivitiEventListener
{
     ...
     * Returning true when {@link #isFailOnException()} is called.
     */
    ...
    /**
     * @return true, if the event is an {@link ActivitiEntityEvent} and (if needed) the entityClass set in this
     *         instance, is assignable from the entity class in the event.
     */
