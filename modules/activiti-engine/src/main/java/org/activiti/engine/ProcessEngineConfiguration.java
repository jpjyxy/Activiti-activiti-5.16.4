 ...
 * (jdbc:h2:tcp://localhost/activiti) in standalone mode. Standalone mode means that Activiti will manage the
 * transactions on the JDBC connections that it creates. One transaction per service method. For a description of how to
 * write the configuration files, see the userguide.
 * </p>
 * 
 * <p>
 ...
 * (jdbc:h2:tcp://localhost/activiti) in standalone mode. The DB schema strategy default is in this case
 * <code>create-drop</code>. Standalone mode means that Activiti will manage the transactions on the JDBC connections
 * that it creates. One transaction per service method.
 * </p>
 * 
 * <p>
 * On all forms of creating a process engine, you can first customize the configuration before calling the
 * {@link #buildProcessEngine()} method by calling any of the setters like this:
 * 
 * <pre>
 * ProcessEngine processEngine = ProcessEngineConfiguration.createProcessEngineConfigurationFromResourceDefault()
...
public abstract class ProcessEngineConfiguration implements EngineServices
{
    ...
    /**
     * Checks the version of the DB schema against the library when the process engine is being created and throws an
     * exception if the versions don't match.
     */
    public static final String DB_SCHEMA_UPDATE_FALSE = "false";
    
    /**
     * Creates the schema when the process engine is being created and drops the schema when the process engine is being
     * closed.
     */
     ...
     * example, if you specify a prefix named 'PRE1.', activiti will query for executions in a table named
     ...
     * {@link ProcessEngineConfiguration#DB_SCHEMA_UPDATE_TRUE}, activiti will create the database tables using the
     ...
     * This is relevant for checking if Activiti-tables exist, the databaseTablePrefix will not be used here - since the
