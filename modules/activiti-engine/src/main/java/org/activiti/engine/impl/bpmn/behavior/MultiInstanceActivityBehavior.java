 ...
 * 
 * Multi instance functionality is implemented as an {@link ActivityBehavior} that wraps the original
 * {@link ActivityBehavior} of the activity.
 * 
 * Only subclasses of {@link AbstractBpmnActivityBehavior} can have multi-instance behavior. As such, special logic is
 * contained in the {@link AbstractBpmnActivityBehavior} to delegate to the {@link MultiInstanceActivityBehavior} if
 * needed.
 * 
 * @author Joram Barrez
 * @author Falko Menge
...
public abstract class MultiInstanceActivityBehavior extends FlowNodeActivityBehavior implements
    CompositeActivityBehavior, SubProcessActivityBehavior
{
    ...
    /**
     * @param innerActivityBehavior The original {@link ActivityBehavior} of the activity that will be wrapped inside
     *            this behavior.
     * @param isSequential Indicates whether the multi instance behavior must be sequential or parallel
     */
    ...
    protected int resolveNrOfInstances(ActivityExecution execution)
    {
        ...
        else if (collectionExpression != null)
        {
            ...
            {
                throw new ActivitiIllegalArgumentException(collectionExpression.getExpressionText()
    ...
    /**
     * Since no transitions are followed when leaving the inner activity, it is needed to call the end listeners
     * yourself.
     */
