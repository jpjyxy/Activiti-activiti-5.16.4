...
public enum ActivitiEventType
{
    ...
    /**
     * An activity is about to be executed as a compensation for another activity. The event targets the activity that
     * is about to be executed for compensation.
     */
    ACTIVITY_COMPENSATE,
    
    /**
     * An activity has received a message event. Dispatched before the actual message has been received by the activity.
     * This event will be either followed by a {@link #ACTIVITY_SIGNALLED} event or {@link #ACTIVITY_COMPLETE} for the
     * involved activity, if the message was delivered successfully.
     */
    ...
    /**
     * A task has been completed. Dispatched before the task entity is deleted ({@link #ENTITY_DELETED}). If the task is
     * part of a process, this event is dispatched before the process moves on, as a result of the task completion. In
     * that case, a {@link #ACTIVITY_COMPLETED} will be dispatched after an event of this type for the activity
     * corresponding to the task.
     */
    TASK_COMPLETED,
    
    /**
     * A process has been completed. Dispatched after the last activity is ACTIVITY_COMPLETED. Process is completed when
     * it reaches state in which process instance does not have any transition to take.
     */
    PROCESS_COMPLETED,
    ...
    /**
     * All memberships in the related group have been deleted. No individual {@link #MEMBERSHIP_DELETED} events will be
     * dispatched due to possible performance reasons. The event is dispatched before the memberships are deleted, so
     * they can still be accessed in the dispatch method of the listener.
     */
    MEMBERSHIPS_DELETED;
