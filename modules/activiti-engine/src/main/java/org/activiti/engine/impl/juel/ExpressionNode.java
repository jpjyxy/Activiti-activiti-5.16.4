...
/**
 * Expression node interface. This interface provides all the methods needed for value expressions and method
 * expressions.
 * 
...
public interface ExpressionNode extends Node
{
    ...
    /**
     * @return <code>true</code> if the subtree rooted at this node could be used as an lvalue expression (identifier or
     *         property sequence with non-literal prefix).
     */
    ...
    /**
     * Get the canonical expression string for this node. Variable and funtion names will be replaced in a way such that
     * two expression nodes that have the same node structure and bindings will also answer the same value here.
     * <p/>
     * For example, <code>"${foo:bar()+2*foobar}"</code> may lead to <code>"${&lt;fn>() + 2 * &lt;var>}"</code> if
     * <code>foobar</code> is a bound variable. Otherwise, the structural id would be
