 ...
 * 
 * <p>
 * The ProcessEngine and the services will be made available to the test class through the getters of the activitiRule.
 * The processEngine will be initialized by default with the activiti.cfg.xml resource on the classpath. To specify a
 * different configuration file, pass the resource location in {@link #ActivitiRule(String) the appropriate constructor}
 ...
 * deployment gets deployed before the setUp and {@link RepositoryService#deleteDeployment(String, boolean) cascade
 * deleted} after the tearDown.
 * </p>
 * 
 * <p>
 * The activitiRule also lets you {@link ActivitiRule#setCurrentTime(Date) set the current time used by the process
 * engine}. This can be handy to control the exact time that is used by the engine in order to verify e.g. e.g. due
 * dates of timers. Or start, end and duration times in the history service. In the tearDown, the internal clock will
 * automatically be reset to use the current system time rather then the time that was set during a test method.
 * </p>
 * 
 * @author Tom Baeyens
 */
public class ActivitiRule implements TestRule
{
    ...
    protected void starting(Description description)
    {
        ...
        {
            deploymentId =
                TestHelper.annotationDeploymentSetUp(processEngine,
