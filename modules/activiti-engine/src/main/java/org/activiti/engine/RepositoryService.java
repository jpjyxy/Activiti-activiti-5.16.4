...
public interface RepositoryService
{
    ...
    /**
     * Sets the category of the deployment. Deployments can be queried by category: see
     * {@link DeploymentQuery#deploymentCategory(String)}.
     * 
     ...
     * @throws ActivitiObjectNotFoundException when the resource doesn't exist in the given deployment or when no
     *             deployment exists for the given deploymentId.
     */
     ...
     * 
     * Changes the tenant identifier of a deployment to match the given tenant identifier. This change will cascade to
     * any related entity: - process definitions related to the deployment - process instances related to those process
     * definitions - executions related to those process instances - tasks related to those process instances - jobs
     * related to the process definitions and process instances
     * 
     * This method can be used in the case that there was no tenant identifier set on the deployment or those entities
     * before.
     * 
     * This method can be used to remove a tenant identifier from the deployment and related entities (simply pass
     * null).
     * 
     * Important: no optimistic locking will be done while executing the tenant identifier change!
     * 
     * This is an experimental feature, mainly because it WILL NOT work properly in a clustered environment without
     * special care: suppose some process instance is in flight. The process definition is in the process definition
     * cache. When a task or job is created when continuing the process instance, the process definition cache will be
     * consulted to get the process definition and from it the tenant identifier. Since it's cached, it will not be the
     * new tenant identifier. This method does clear the cache for this engineinstance , but it will not be cleared on
     * other nodes in a cluster (unless using a shared process definition cache).
     * 
     ...
     * 
     * If a process definition is in state suspended, it will not be possible to start new process instances based on
     * the process definition.
     * 
     * <strong>Note: all the process instances of the process definition will still be active (ie. not
     * suspended)!</strong>
     * 
     ...
     * the process definition.
     * 
     * @param suspendProcessInstances If true, all the process instances of the provided process definition will be
     *            suspended too.
     * @param suspensionDate The date on which the process definition will be suspended. If null, the process definition
     *            is suspended immediately. Note: The job executor needs to be active to use this!
     * 
     * @throws ActivitiObjectNotFoundException if no such processDefinition can be found.
     * @throws ActivitiException if the process definition is already in state suspended.
     */
    ...
    /**
     * Similar to {@link #suspendProcessDefinitionByKey(String, boolean, Date)}, but only applicable for the given
     * tenant identifier.
     */
     ...
     * 
     * @throws ActivitiObjectNotFoundException if no such processDefinition can be found or if the process definition is
     *             already in state active.
     */
     ...
     * Activates the process definition with the given id.
     * 
     * @param activationDate The date on which the process definition will be activated. If null, the process definition
     *            is suspended immediately. Note: The job executor needs to be active to use this!
     * 
     * @throws ActivitiObjectNotFoundException if no such processDefinition can be found.
     ...
     * @throws ActivitiException if the process definition is already in state active.
     */
    void activateProcessDefinitionByKey(String processDefinitionKey, boolean activateProcessInstances,
        Date activationDate);
    ...
    /**
     * Similar to {@link #activateProcessDefinitionByKey(String, boolean, Date)}, but only applicable for the given
     * tenant identifier.
     */
    void activateProcessDefinitionByKey(String processDefinitionKey, boolean activateProcessInstances,
        Date activationDate, String tenantId);
    
    /**
     * Sets the category of the process definition. Process definitions can be queried by category: see
     ...
     * @throws ActivitiObjectNotFoundException when the process diagram doesn't exist.
     */
    InputStream getProcessDiagram(String processDefinitionId);
    
    /**
     * Returns the {@link ProcessDefinition} including all BPMN information like additional Properties (e.g.
     * documentation).
     */
     ...
     * The {@link BpmnModel} is a pojo versions of the BPMN 2.0 xml and can be used to introspect the process definition
     * using regular Java.
     */
     ...
     * {@link RepositoryService#getProcessDiagram(String)}.
     * 
    ...
    /**
     * @param modelId id of model to delete, cannot be null. When an id is passed for an unexisting model, this
    ...
    /**
     * Retrieves the {@link IdentityLink}s associated with the given process definition. Such an {@link IdentityLink}
    ...
    /**
     * Validates the given process definition against the rules for executing a process definition on the Activiti
     * engine.
     * 
     * To create such a {@link BpmnModel} from a String, following code may be used:
     * 
     * XMLInputFactory xif = XMLInputFactory.newInstance(); InputStreamReader in = new InputStreamReader(new
     * ByteArrayInputStream(myProcess.getBytes()), "UTF-8"); // Change to other streams for eg from classpath
     * XMLStreamReader xtr = xif.createXMLStreamReader(in); bpmnModel = new BpmnXMLConverter().convertToBpmnModel(xtr);
     * 
     */
