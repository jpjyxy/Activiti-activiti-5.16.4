 ...
 * 
 * These operations will typically not be used in a workflow driven application, but are used in for example the
 * operational console.
 * 
 * @author Tom Baeyens
 * @author Joram Barrez
...
public interface ManagementService
{
    ...
    /**
     * Gets the metadata (column names, column types, etc.) of a certain table. Returns null when no table exists with
     * the given name.
     */
    TableMetaData getTableMetaData(String tableName);
    
    /**
     * Creates a {@link TablePageQuery} that can be used to fetch {@link TablePage} containing specific sections of
     * table row data.
     */
    ...
    /**
     * Forced synchronous execution of a job (eg. for administation or testing) The job will be executed, even if the
     * process definition and/or the process instance is in suspended state.
     * 
     ...
     * 
     * Whenever the JobExecutor fails to execute a job, this value is decremented. When it hits zero, the job is
     * supposed to be dead and not retried again. In that case, this method can be used to increase the number of
     * retries.
     * 
     ...
     * [EXPERIMENTAL]
     * 
     * Returns a list of event log entries, describing everything the engine has processed. Note that the event logging
     * must specifically must be enabled in the process engine configuration.
     * 
     ...
     * [EXPERIMENTAL]
     * 
     * Returns a list of event log entries for a specific process instance id. Note that the event logging must
     * specifically must be enabled in the process engine configuration.
     * 
     * Passing null as arguments will effectively fetch ALL event log entries. Be careful, as this list might be huge!
     */
    ...
    /**
     * Delete a EventLogEntry. Typically only used in testing, as deleting log entries defeats the whole purpose of
     * keeping a log.
     */
