 ...
 * 
 * This class is also used as an EL "service provider". The <em>JUEL</em> jar file specifies this class as el expression
 * factory implementation in <code>META-INF/services/javax.el.ExpressionFactory</code>. Calling
 * {@link ExpressionFactory#newInstance()} will then return an instance of this class, configured as described below.
 * 
 ...
 * <code>javax.el.ExpressionFactory</code> whose value is the name of this class, these properties are taken as default
 * properties.</li>
 * <li>Otherwise, if system property <code>javax.el.ExpressionFactory</code> is set to the name of this class, the
 * system properties {@link System#getProperties()} are taken as default properties.</li>
 * <li>
 ...
 * <li>
 * <code>javax.el.methodInvocations</code> - allow method invocations as in <code>${foo.bar(baz)}</code> (boolean,
 * default is <code>false</code>).</li>
 * <li>
 * <code>javax.el.nullProperties</code> - resolve <code>null</code> properties as in <code>${foo[null]}</code> (boolean,
 * default is <code>false</code>).</li>
 * <li>
 * <code>javax.el.varArgs</code> - support function/method calls using varargs (boolean, default is <code>false</code>).
 * </li>
 * </ul>
 * 
...
{
    /**
     * A profile provides a default set of language features that will define the builder's behavior. A profile can be
     * adjusted using the <code>javax.el.methodInvocations</code>, <code>javax.el.varArgs</code> and
     * <code>javax.el.nullProperties</code> properties.
     * 
     * @since 2.2
     */
    ...
    /**
     * Create a new expression factory using the default builder and cache implementations. The builder and cache are
     * configured from <code>el.properties</code> (see above). The maximum cache size will be 1000 unless overridden in
     * <code>el.properties</code>. The builder profile is {@link Profile#JEE6} (features may be overridden in
    ...
    /**
     * Create a new expression factory using the default builder and cache implementations. The builder and cache are
     * configured from the specified profile and <code>el.properties</code> (see above). The maximum cache size will be
     * 1000 unless overridden in <code>el.properties</code>.
     * 
    ...
    /**
     * Create a new expression factory using the default builder and cache implementations. The builder and cache are
     * configured using the specified properties. The maximum cache size will be 1000 unless overridden by property
     * <code>javax.el.cacheSize</code>. The builder profile is {@link Profile#JEE6} (features may be overridden in
    ...
    /**
     * Create a new expression factory using the default builder and cache implementations. The builder and cache are
     * configured using the specified profile and properties. The maximum cache size will be 1000 unless overridden by
     ...
     * Create a new expression factory using the default builder and cache implementations. The builder and cache are
     * configured using the specified properties. The maximum cache size will be 1000 unless overridden by property
     * <code>javax.el.cacheSize</code>. The builder profile is {@link Profile#JEE6} (individual features may be
     * overridden in <code>properties</code>).
     * 
     * @param properties used to initialize this factory (may be <code>null</code>)
    ...
    /**
     * Create the factory's tree store. This implementation creates a new tree store using the default builder and cache
     * implementations. The builder and cache are configured using the specified properties. The maximum cache size will
     * be as specified unless overridden by property <code>javax.el.cacheSize</code>.
     */
    ...
    /**
     * Create the factory's type converter. This implementation takes the <code>de.odysseus.el.misc.TypeConverter</code>
     * property as the name of a class implementing the <code>de.odysseus.el.misc.TypeConverter</code> interface. If the
     * property is not set, the default converter (<code>TypeConverter.DEFAULT</code>) is used.
     */
    ...
    /**
     * Create the factory's builder. This implementation takes the <code>de.odysseus.el.tree.TreeBuilder</code> property
     * as a name of a class implementing the <code>de.odysseus.el.tree.TreeBuilder</code> interface. If the property is
     * not set, a plain <code>de.odysseus.el.tree.impl.Builder</code> is used. If the configured class is a subclass of
     * <code>de.odysseus.el.tree.impl.Builder</code> and which provides a constructor taking an array of
     * <code>Builder.Feature</code>, this constructor will be invoked. Otherwise, the default constructor will be used.
     */
