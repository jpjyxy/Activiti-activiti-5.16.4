...
public class AcquireJobsCmd implements Command<AcquiredJobs>
{
    ...
    public AcquiredJobs execute(CommandContext commandContext)
    {
        ...
        List<JobEntity> jobs =
            commandContext.getJobEntityManager().findNextJobsToExecute(new Page(0, maxNonExclusiveJobsPerAcquisition));
        ...
        for (JobEntity job : jobs)
        {
            ...
            if (job != null && !acquiredJobs.contains(job.getId()))
            {
                if (job.isExclusive() && job.getProcessInstanceId() != null)
                {
                    ...
                    List<JobEntity> exclusiveJobs =
                        commandContext.getJobEntityManager().findExclusiveJobsToExecute(job.getProcessInstanceId());
