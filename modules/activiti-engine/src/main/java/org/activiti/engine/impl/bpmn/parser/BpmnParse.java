...
public class BpmnParse implements BpmnXMLConstants
{
     ...
     * messages.
     * 
     * All the map's elements are defined outside the process definition(s), which means that this map doesn't need to
     * be re-initialized for each new process definition.
     */
    ...
    {
        this.itemDefinitions.put("http://www.w3.org/2001/XMLSchema:string", new ItemDefinition(
    ...
    protected void setStreamSource(StreamSource streamSource)
    {
        ...
        {
            throw new ActivitiIllegalArgumentException("invalid: multiple sources " + this.streamSource + " and "
                + streamSource);
        }
    ...
    protected XMLImporter getImporter(Import theImport)
    {
        ...
        else
        {
            if (theImport.getImportType().equals("http://schemas.xmlsoap.org/wsdl/"))
            {
                ...
                {
                    wsdlImporterClass =
                        Class.forName("org.activiti.engine.impl.webservice.CxfWSDLImporter",
                            true,
        ...
        for (Interface interfaceObject : bpmnModel.getInterfaces())
        {
            ...
            for (org.activiti.bpmn.model.Operation operationObject : interfaceObject.getOperations())
            {
                if (this.messages.containsKey(operationObject.getInMessageRef()))
                {
                    ...
                    Operation operation =
                        new Operation(operationObject.getId(), operationObject.getName(), bpmnInterface, inMessage);
            ...
            for (String bpmnReference : bpmnModel.getLocationMap().keySet())
            {
                if (bpmnModel.getFlowElement(bpmnReference) == null)
                {
                    ...
                    if (bpmnModel.getArtifact(bpmnReference) == null)
                    {
                        ...
                        {
                            LOGGER.warn("Invalid reference in diagram interchange definition: could not find "
                                + bpmnReference);
                        }
                    }
                }
                ...
                {
                    LOGGER.warn("Invalid reference in diagram interchange definition: " + bpmnReference
