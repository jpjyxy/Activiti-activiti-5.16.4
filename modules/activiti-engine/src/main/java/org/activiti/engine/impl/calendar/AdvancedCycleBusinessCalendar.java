...
/**
 * An Activiti BusinessCalendar for cycle based schedules that takes into account a different daylight savings time zone
 * than the one that the server is configured for.
 * <p>
 * For CRON strings DSTZONE is used as the time zone that the CRON schedule refers to. Leave it out to use the server
 * time zone.
 * <p>
 * For ISO strings the time zone offset for the date/time specified is part of the string itself. DSTZONE is used to
 * determine what the offset should be NOW, which may be different than when the workflow was scheduled if it is
 ...
 * <p>
 * Schedule strings are versioned. Version 1 strings will use the original Activiti CycleBusinessCalendar. All new
 * properties are ignored. Version 2 strings will use the new daylight saving time logic.
 * 
 * <pre>
 *   For example:
 ...
 * </pre>
 * 
 * By default (if no VER key is included in the string), it assumes version 2. This can be changed by modifying the
 * defaultScheduleVersion property.
 * <p>
 * 
 * @author mseiden
 */
public class AdvancedCycleBusinessCalendar extends CycleBusinessCalendar
{
    ...
    public Date resolveDuedate(String duedateDescription)
    {
        ...
        try
        {
            ...
            Date date =
                resolvers.get(version == null ? getDefaultScheduleVersion() : Integer.valueOf(version))
                    .resolve(duedateDescription,
                        clockReader,
