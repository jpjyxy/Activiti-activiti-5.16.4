...
public class ProcessDiagramLayoutFactory
{
     ...
     * @param imageStream BPMN 2.0 diagram in PNG format (JPEG and other formats supported by {@link ImageIO} may also
     *            work)
     ...
     * {@link RepositoryService#getProcessDiagram(String)}.
     * 
     * @param bpmnXmlStream BPMN 2.0 XML document
     * @param imageStream BPMN 2.0 diagram in PNG format (JPEG and other formats supported by {@link ImageIO} may also
     *            work)
     * @return Layout of the process diagram
     * @return null when parameter imageStream is null
    ...
    public DiagramLayout getBpmnProcessDiagramLayout(Document bpmnModel, InputStream imageStream)
    {
        ...
        Map<String, DiagramElement> listOfBoundsForImage =
            transformBoundsForImage(diagramBoundsImage, diagramBoundsXml, listOfBounds);
    ...
    protected Map<String, DiagramNode> getElementBoundsFromBpmnDi(Document bpmnModel)
    {
        ...
        for (int i = 0; i < shapes.getLength(); i++)
        {
            ...
            for (int j = 0; j < childNodes.getLength(); j++)
            {
                ...
                if (childNode instanceof Element && BpmnParser.BPMN_DC_NS.equals(childNode.getNamespaceURI())
    ...
    protected Map<String, DiagramElement> transformBoundsForImage(DiagramNode diagramBoundsImage,
        DiagramNode diagramBoundsXml, Map<String, DiagramNode> listOfBounds)
    {
        ...
        {
            listOfBoundsForImage.put(bounds.getKey(),
                transformBoundsForImage(diagramBoundsImage, diagramBoundsXml, bounds.getValue()));
        }
        return listOfBoundsForImage;
    }
    
    protected DiagramNode transformBoundsForImage(DiagramNode diagramBoundsImage, DiagramNode diagramBoundsXml,
        DiagramNode elementBounds)
    {
        ...
        elementBoundsForImage.setX((double)Math.round((elementBounds.getX() - diagramBoundsXml.getX()) * scalingFactorX
            + diagramBoundsImage.getX()));
        elementBoundsForImage.setY((double)Math.round((elementBounds.getY() - diagramBoundsXml.getY()) * scalingFactorY
            + diagramBoundsImage.getY()));
