...
public interface UserQuery extends Query<UserQuery, User>
{
    ...
    /**
     * Only select {@link User}s where the first name matches the given parameter. The syntax is that of SQL, eg.
     * %activivi%.
     */
    ...
    /**
     * Only select {@link User}s where the last name matches the given parameter. The syntax is that of SQL, eg.
     * %activivi%.
     */
    UserQuery userLastNameLike(String lastNameLike);
    
    /**
     * Only select {@link User}s where the full name matches the given parameters. Both the first name and last name
     * will be tried, ie in semi-sql: where firstName like xxx or lastname like xxx
     */
