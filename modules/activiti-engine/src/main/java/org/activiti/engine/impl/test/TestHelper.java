...
public abstract class TestHelper
{
    ...
    public static void annotationDeploymentTearDown(ProcessEngine processEngine, String deploymentId,
    ...
    /**
     * get a resource location by convention based on a class (type) and a relative resource name. The return value will
     * be the full classpath location of the type, plus a suffix built from the name parameter:
     * <code>BpmnDeployer.BPMN_RESOURCE_SUFFIXES</code>. The first resource matching a suffix will be returned.
     */
    ...
    /**
     * Each test is assumed to clean up all DB content it entered. After a test method executed, this method scans all
     * tables to see if the DB is completely clean. It throws AssertionFailed in case the DB is not clean. If the DB is
     * not clean, it is cleaned by performing a create a drop.
     */
