...
public interface TaskInfoQuery<T extends TaskInfoQuery<?, ?>, V extends TaskInfo> extends Query<T, V>
{
     ...
     * 
     * @throws ActivitiIllegalArgumentException When passed name list is empty or <code>null</code> or contains
     ...
     * Only select tasks with a name that is in the given list
     * 
     * This method, unlike the {@link #taskNameIn(List)} method will not take in account the upper/lower case: both the
     * input parameters as the column value are lowercased when the query is executed.
     * 
     * @throws ActivitiIllegalArgumentException When passed name list is empty or <code>null</code> or contains
     *             <code>null String</code>.
     */
    ...
    /**
     * Only select tasks with a name matching the parameter. The syntax is that of SQL: for example usage:
     * nameLike(%activiti%)
     */
     ...
     * nameLike(%activiti%)
     * 
     * This method, unlike the {@link #taskNameLike(String)} method will not take in account the upper/lower case: both
     * the input parameter as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks with a description matching the parameter . The syntax is that of SQL: for example usage:
     * descriptionLike(%activiti%)
     */
     ...
     * descriptionLike(%activiti%)
     * 
     * This method, unlike the {@link #taskDescriptionLike(String)} method will not take in account the upper/lower
     * case: both the input parameter as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks which were last assigned to an assignee like the given value. The syntax that should be used is
     * the same as in SQL, eg. %activiti%.
     */
     ...
     * the same as in SQL, eg. %activiti%.
     * 
     * This method, unlike the {@link #taskAssigneeLike(String)} method will not take in account the upper/lower case:
     * both the input parameter as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks which were last assigned to an owner like the given value. The syntax that should be used is
     * the same as in SQL, eg. %activiti%.
     */
     ...
     * the same as in SQL, eg. %activiti%.
     * 
     * This method, unlike the {@link #taskOwnerLike(String)} method will not take in account the upper/lower case: both
     * the input parameter as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks for which there exist an {@link IdentityLink} with the given user, including tasks which have
     * been assigned to the given user (assignee) or owned by the given user (owner).
     */
     ...
     * 
     * @throws ActivitiIllegalArgumentException When query is executed and {@link #taskCandidateGroup(String)} or
     *             {@link #taskCandidateUser(String)} has been executed on the query instance. When passed group list is
    ...
    /**
     * Only select tasks with a business key like the given value The syntax is that of SQL: for example usage:
     ...
     * processInstanceBusinessKeyLike("%activiti%").
     * 
     * This method, unlike the {@link #processInstanceBusinessKeyLike(String)} method will not take in account the
     * upper/lower case: both the input parameter as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks with the given taskDefinitionKey. The task definition key is the id of the userTask:
     * &lt;userTask id="xxx" .../&gt;
     **/
    T taskDefinitionKey(String key);
    
    /**
     * Only select tasks with a taskDefinitionKey that match the given parameter. The syntax is that of SQL: for example
     * usage: taskDefinitionKeyLike("%activiti%"). The task definition key is the id of the userTask: &lt;userTask
     * id="xxx" .../&gt;
     **/
     ...
     * The syntax that should be used is the same as in SQL, eg. %activiti%.
     * 
     * This method, unlike the {@link #processDefinitionKeyLike(String)} method will not take in account the upper/lower
     * case: both the input parameter as the column value are lowercased when the query is executed.
     */
     ...
     * <p>
     * This method only works if your database has encoding/collation that supports case-sensitive queries. For example,
     * use "collate UTF-8" on MySQL and for MSSQL, select one of the case-sensitive Collations available (<a
     * href="http://msdn.microsoft.com/en-us/library/ms144250(v=sql.105).aspx">MSDN Server Collation Reference</a>).
     * </p>
     */
    T taskVariableValueEqualsIgnoreCase(String name, String value);
    
    /**
     * Only select tasks which have a local task variable with the given name, but with a different value than the
     * passed value. Byte-arrays and {@link Serializable} objects (which are not primitive type wrappers) are not
     * supported.
     */
    ...
    /**
     * Only select tasks which have a local variable value greater than the passed value when they ended. Booleans,
    ...
    /**
     * Only select tasks which have a local variable value less than the passed value when the ended.Booleans,
     * Byte-arrays and {@link Serializable} objects (which are not primitive type wrappers) are not supported.
     * 
     * @param name cannot be null.
     * @param value cannot be null.
    ...
    /**
     * Only select tasks which have a local variable value like the given value when they ended. This can be used on
     * string variables only.
     * 
     * @param name cannot be null.
     * @param value cannot be null. The string can include the wildcard character '%' to express like-strategy: starts
    ...
    /**
     * Only select tasks which are part of a process that has a local string variable which is not the given value, case
     * insensitive.
     * <p>
     * This method only works if your database has encoding/collation that supports case-sensitive queries. For example,
     * use "collate UTF-8" on MySQL and for MSSQL, select one of the case-sensitive Collations available (<a
    ...
    /**
     * Only select tasks which have a global variable value greater than the passed value when they ended. Booleans,
     * Byte-arrays and {@link Serializable} objects (which are not primitive type wrappers) are not supported.
     * 
     * @param name cannot be null.
     * @param value cannot be null.
    ...
    /**
     * Only select tasks which have a global variable value less than the passed value when the ended.Booleans,
     * Byte-arrays and {@link Serializable} objects (which are not primitive type wrappers) are not supported.
     * 
     * @param name cannot be null.
     * @param value cannot be null.
    ...
    /**
     * Only select tasks which have a global variable value like the given value when they ended. This can be used on
     * string variables only.
     * 
     * @param name cannot be null.
     * @param value cannot be null. The string can include the wildcard character '%' to express like-strategy: starts
    ...
    /**
     * All query clauses called will be added to a single or-statement. This or-statement will be included with the
     * other already existing clauses in the query, joined by an 'and'.
     * 
     * Calling endOr() will add all clauses to the regular query again. Calling or() after endOr() has been called will
    ...
    /**
     * Order by due date (needs to be followed by {@link #asc()} or {@link #desc()}). If any of the tasks have null for
     * the due date, these will be first in the result.
     */
    T orderByDueDateNullsFirst();
    
    /**
     * Order by due date (needs to be followed by {@link #asc()} or {@link #desc()}). If any of the tasks have null for
     * the due date, these will be last in the result.
     */
