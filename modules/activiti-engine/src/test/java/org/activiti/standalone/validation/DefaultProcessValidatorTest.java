...
public class DefaultProcessValidatorTest
{
        ...
        throws Exception
    {
        ...
        InputStream xmlStream =
            this.getClass()
                .getClassLoader()
                .getResourceAsStream("org/activiti/engine/test/validation/invalidProcess.bpmn20.xml");
    ...
    {
        String flowWithoutConditionNoDefaultFlow =
        ...
        InputStreamReader in =
            new InputStreamReader(new ByteArrayInputStream(flowWithoutConditionNoDefaultFlow.getBytes()), "UTF-8");
        XMLStreamReader xtr = xif.createXMLStreamReader(in);
        BpmnModel bpmnModel = new BpmnXMLConverter().convertToBpmnModel(xtr);
        Assert.assertNotNull(bpmnModel);
        List<ValidationError> allErrors = processValidator.validate(bpmnModel);
     ...
     * Test for https://jira.codehaus.org/browse/ACT-2071:
     * 
     * If there is at least one process definition which is executable, and the deployment contains other process
     * definitions which are not executable, then add a warning for those non executable process definitions
     */
    @Test
    ...
    protected List<ValidationError> findErrors(List<ValidationError> errors, String validatorSetName,
