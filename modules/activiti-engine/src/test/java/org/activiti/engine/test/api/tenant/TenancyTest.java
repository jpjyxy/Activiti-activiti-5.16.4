...
public class TenancyTest extends PluggableActivitiTestCase
{
    ...
    public void testProcessDefinitionTenancy()
    {
        ...
        // Extra check: we deployed the one task process twice, but once with tenant and once without. The latest query
        // should show this.
        assertEquals(processDefinitionIdWithTenant2, repositoryService.createProcessDefinitionQuery()
        ...
        assertEquals(0, repositoryService.createProcessDefinitionQuery()
            ...
            .latestVersion()
            .count());
        assertEquals(processDefinitionIdWithoutTenant, repositoryService.createProcessDefinitionQuery()
            .processDefinitionKey("oneTaskProcess")
            .processDefinitionWithoutTenantId()
            .latestVersion()
            .singleResult()
    ...
    public void testProcessInstanceTenancy()
    {
        ...
        assertEquals(nrOfProcessInstancesNoTenant, runtimeService.createProcessInstanceQuery()
            .processInstanceWithoutTenantId()
            .list()
            .size());
        assertEquals(nrOfProcessInstancesWithTenant, runtimeService.createProcessInstanceQuery()
    ...
    public void testExecutionTenancy()
    {
        ...
        assertEquals(TEST_TENANT_ID, runtimeService.createExecutionQuery()
            .processDefinitionId(processDefinitionId)
            .list()
            .get(0)
            .getTenantId());
        assertEquals("", runtimeService.createExecutionQuery()
        ...
        assertEquals(3 * nrOfProcessInstancesNoTenant, runtimeService.createExecutionQuery()
    ...
    public void testTaskTenancy()
    {
        ...
        // Check the query results
        assertEquals(TEST_TENANT_ID, taskService.createTaskQuery()
            .processDefinitionId(processDefinitionIdWithTenant)
            .list()
            .get(0)
            .getTenantId());
    ...
    public void testModelTenancy()
    {
        ...
        assertEquals(nrOfModelsWithTenant, repositoryService.createModelQuery()
    ...
    public void testChangeDeploymentTenantId()
    {
        ...
        // Verify tasks
        assertEquals(2 * (nrOfProcessInstancesNoTenant + nrOfProcessInstancesWithTenant), taskService.createTaskQuery()
            .list()
            .size());
        assertEquals(0, taskService.createTaskQuery().taskTenantId(TEST_TENANT_ID).list().size());
        assertEquals(2 * nrOfProcessInstancesWithTenant, taskService.createTaskQuery()
            .taskTenantId(newTenantId)
            .list()
            .size());
        assertEquals(2 * nrOfProcessInstancesNoTenant, taskService.createTaskQuery()
            .taskWithoutTenantId()
            .list()
            .size());
    ...
    public void testHistoryTenancy()
    {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT))
        {
            ...
            assertEquals(nrOfProcessInstancesWithTenant, historyService.createHistoricProcessInstanceQuery()
                .processInstanceTenantId(TEST_TENANT_ID)
                .list()
                .size());
            assertEquals(nrOfProcessInstancesWithTenant, historyService.createHistoricProcessInstanceQuery()
                ...
                .list()
                .size());
            assertEquals(nrOfProcessInstancesNoTenant, historyService.createHistoricProcessInstanceQuery()
                .processInstanceWithoutTenantId()
                .list()
                .size());
            ...
            assertEquals(nrOfProcessInstancesWithTenant, historyService.createHistoricTaskInstanceQuery()
                .taskTenantIdLike("my%")
                .list()
                .size());
            assertEquals(nrOfProcessInstancesNoTenant, historyService.createHistoricTaskInstanceQuery()
                .taskWithoutTenantId()
                .list()
                .size());
            ...
            assertEquals(3 * nrOfProcessInstancesWithTenant, historyService.createHistoricActivityInstanceQuery()
            ...
            assertEquals(3 * nrOfProcessInstancesWithTenant, historyService.createHistoricActivityInstanceQuery()
                ...
                .list()
                .size());
            assertEquals(3 * nrOfProcessInstancesNoTenant, historyService.createHistoricActivityInstanceQuery()
    ...
    public void testProcessDefinitionKeyClashBetweenTenants()
    {
        ...
        // verify query
        assertEquals("oneTaskProcess", repositoryService.createProcessDefinitionQuery()
        ...
        try
        {
            ...
            fail(); // shouldnt happen, there is no process definition with that key that has no tenant, it has to give
                    // an exception as such!
        }
        catch (Exception e)
        {
        }
    ...
    public void testSignalFromProcessTenancy()
    {
        ...
        // Now, start 1 process instance that fires a signal event (not in tenant context), it should obly continue
        // those without tenant
    ...
    public void testCallActivityWithTenant()
    {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY))
        {
            ...
            // Starting Process 1. Process 1 will be executed successfully but when the call to process 2 is made
            // internally it will throw the exception
            ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKeyAndTenantId("process1",
                    null,
                    CollectionUtil.singletonMap("sendFor", "test"),
                    tenantId);
            Assert.assertNotNull(processInstance);
            
            Assert.assertEquals(1, historyService.createHistoricProcessInstanceQuery()
            ...
            // following line if executed will give activiti object not found exception as the process1 is linked to a
            // tenant id.
            try
            {
