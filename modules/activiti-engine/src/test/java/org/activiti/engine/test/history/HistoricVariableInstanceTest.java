...
public class HistoricVariableInstanceTest extends PluggableActivitiTestCase
{
        ...
        throws Exception
    {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL))
        {
            ...
            /*
             * This is OK! The variable is set on the root execution, on a execution never run through the activity,
             * where the process instances stands when calling the set Variable. But the ActivityId of this flow node is
             * used. So the execution id's doesn't have to be equal.
             * 
    ...
    public void testNativeHistoricVariableInstanceQuery()
    {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL))
        {
            ...
            assertEquals("test123", historyService.createNativeHistoricVariableInstanceQuery()
            ...
            assertEquals(2, historyService.createNativeHistoricVariableInstanceQuery()
                .sql(sqlWithConditions)
                .parameter("name", "var%")
                .list()
                .size());
            ...
            // paging
            assertEquals(3, historyService.createNativeHistoricVariableInstanceQuery()
                .sql(baseQuerySql)
                .listPage(0, 3)
                .size());
            assertEquals(2, historyService.createNativeHistoricVariableInstanceQuery()
    ...
    public void testNativeHistoricDetailQuery()
    {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL))
        {
            ...
            assertNotNull(historyService.createNativeHistoricDetailQuery()
                .sql(sqlWithConditions)
                .parameter("name", "myVar")
            ...
            sqlWithConditions = baseQuerySql + " where NAME_ like #{name}";
            assertEquals(2,
                historyService.createNativeHistoricDetailQuery()
                    .sql(sqlWithConditions)
                    .parameter("name", "var%")
                    .list()
                    .size());
            ...
            assertEquals(2,
                historyService.createNativeHistoricDetailQuery()
                    .sql(countSql)
                    .parameter("type", "FormProperty")
                    .parameter("pid", processInstance.getId())
                    .count());
