...
public class SignalEventTest extends PluggableActivitiTestCase
{
    ...
    public void testUseSignalForExceptionsBetweenParallelPaths()
    {
        ...
        taskService.complete(task.getId(),
            CollectionUtil.singletonMap("developers", Arrays.asList("developerOne", "developerTwo", "developerThree")));
        
        // Should be three distinct tasks for each developer
        assertEquals("Develop specifications", taskService.createTaskQuery()
    ...
    public void testSignalWithProcessInstanceScope()
    {
        ...
        assertEquals("userTaskWithSignalCatch",
    ...
    public void testSignalWithGlobalScope()
    {
        ...
        // Since the signal is process instance scoped, the second process shouldn't have proceeded in any way
        assertEquals("userTaskAfterSignalCatch",
            taskService.createTaskQuery().processInstanceId(processInstanceCatch.getId()).singleResult().getName());
    }
    ...
    public void testSignalUserTask()
    {
        ...
        Execution execution =
            runtimeService.createExecutionQuery().processInstanceId(pi.getId()).activityId("waitState").singleResult();
    ...
    public void testSignalStartEventFromProcess()
    {
        ...
        // Starting the process that fires the signal should start three process instances that are listening on that
        // signal
        runtimeService.startProcessInstanceByKey("processWithSignalThrow");
    ...
    public void testSignalStartEventFromProcesAsync()
    {
        ...
        // Starting the process that fires the signal should start 1 process instance that are listening on that signal,
        // the others are done async
        runtimeService.startProcessInstanceByKey("processWithSignalThrow");
