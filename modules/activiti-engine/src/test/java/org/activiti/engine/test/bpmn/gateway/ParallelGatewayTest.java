...
public class ParallelGatewayTest extends PluggableActivitiTestCase
{
    ...
    /**
     * Case where there is a parallel gateway that splits into 3 paths of execution, that are immediately joined,
     * without any wait states in between. In the end, no executions should be in the database.
     */
    @Deployment
    ...
    public void testReceyclingExecutionWithCallActivity()
    {
        ...
        // we complete the task from the parent process, the root execution is receycled, the task in the sub process is
        // still there
        taskService.complete(tasks.get(1).getId());
        tasks = query.list();
        assertEquals(1, tasks.size());
