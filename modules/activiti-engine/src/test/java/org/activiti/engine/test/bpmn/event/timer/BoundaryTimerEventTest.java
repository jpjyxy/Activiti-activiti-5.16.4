...
public class BoundaryTimerEventTest extends PluggableActivitiTestCase
{
     ...
     * 
     * Configuration: - timer 1 -> 2 hours -> secondTask - timer 2 -> 1 hour -> thirdTask - timer 3 -> 3 hours ->
     * fourthTask
     * 
    ...
    public void testTimerOnNestingOfSubprocesses()
    {
        ...
        processEngineConfiguration.getClock().setCurrentTime(new Date(testStartTime.getTime()
            + ((2 * 60 * 60 * 1000) + 5000)));
    ...
    {
        // make sure that if a PI completes in single transaction, JobEntities associated with the execution are
        // deleted.
