...
public class CallActivityAdvancedTest extends PluggableActivitiTestCase
{
    ...
    public void testCallSimpleSubProcess()
    {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY))
        {
            ...
            Set<String> expectedActivities =
                new HashSet<String>(Arrays.asList(new String[] {"theStart", "task", "theEnd"}));
    ...
    /**
     * Test case for a possible tricky case: reaching the end event of the subprocess leads to an end event in the super
     * process instance.
     */
    @Deployment(resources = {
    ...
    public void testTimerOnCallActivity()
    {
        ...
        processEngineConfiguration.getClock().setCurrentTime(new Date(startTime.getTime() + (6 * 60 * 1000))); // + 6
                                                                                                               // minutes,
                                                                                                               // timer
                                                                                                               // fires
                                                                                                               // on 5
    ...
    public void testSubProcessWithDataInputOutput()
    {
        ...
        assertEquals("Hello from the super process.",
            taskService.getVariable(taskBeforeSubProcess.getId(), "subVariable"));
        
        runtimeService.setVariable(taskBeforeSubProcess.getProcessInstanceId(),
            "subVariable",
            "Hello from sub process.");
        
        // super variable is unchanged
        assertEquals("Hello from the super process.",
            runtimeService.getVariable(processInstance.getId(), "superVariable"));
     ...
     */
    @Deployment(resources = {"org/activiti/engine/test/bpmn/callactivity/CallActivity.testTwoSubProcesses.bpmn20.xml",
        "org/activiti/engine/test/bpmn/callactivity/simpleSubProcess.bpmn20.xml"})
