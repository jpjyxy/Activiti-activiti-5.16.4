...
public class InclusiveGatewayTest extends PluggableActivitiTestCase
{
    ...
    {
        ProcessInstance pi =
            runtimeService.startProcessInstanceByKey("partialInclusiveGwMerging",
    ...
    public void testNoSequenceFlowSelected()
    {
        ...
        {
            assertTextPresent("No outgoing sequence flow of the inclusive gateway 'inclusiveGw' could be selected for continuing the process",
                e.getMessage());
        }
    }
    ...
    public void testDecideBasedOnListOrArrayOfBeans()
    {
        ...
        try
        {
            runtimeService.startProcessInstanceByKey("inclusiveDecisionBasedOnListOrArrayOfBeans",
    ...
    {
        ProcessInstance pi =
            runtimeService.startProcessInstanceByKey("inclusiveNoIdOnSequenceFlow",
                CollectionUtil.singletonMap("input", 3));
        Task task = taskService.createTaskQuery().processInstanceId(pi.getId()).singleResult();
     ...
     * 
     * In case of loops, special care needs to be taken in the algorithm, or else stackoverflows will happen very
     * quickly.
     */
    @Deployment
    ...
    {
        ProcessInstance pi =
            runtimeService.startProcessInstanceByKey("inclusiveTestLoop", CollectionUtil.singletonMap("counter", 1));
        ...
        assertEquals("Found executions: " + runtimeService.createExecutionQuery().list(),
            0,
    ...
    /*
     * @Deployment public void testAsyncBehavior() { for (int i = 0; i < 100; i++) { ProcessInstance processInstance =
     * runtimeService.startProcessInstanceByKey("async"); } assertEquals(200,
     * managementService.createJobQuery().count()); waitForJobExecutorToProcessAllJobs(120000, 5000); assertEquals(0,
