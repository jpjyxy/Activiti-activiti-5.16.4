...
public class SubProcessTest extends PluggableActivitiTestCase
{
    ...
    /**
     * A test case that has a timer attached to the subprocess, where 2 concurrent paths are defined when the timer
     * fires.
     */
    @Deployment
    ...
    public void testSimpleParallelSubProcessWithTimer()
    {
        ...
        List<Task> subProcessTasks =
            taskService.createTaskQuery().processInstanceId(processInstance.getId()).orderByTaskName().asc().list();
    ...
    public void testTwoNestedSubProcessesInParallelWithTimer()
    {
        ...
        // processEngineConfiguration.getClock().setCurrentTime(new Date(startTime.getTime() + (2 * 60 * 60 * 1000 ) +
        // 1000));
        // waitForJobExecutorToProcessAllJobs(5000L, 50L);
        Job job = managementService.createJobQuery().singleResult();
        managementService.executeJob(job.getId());
