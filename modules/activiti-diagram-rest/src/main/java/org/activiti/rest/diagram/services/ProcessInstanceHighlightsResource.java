...
public class ProcessInstanceHighlightsResource
{
    ...
    public ObjectNode getHighlighted(@PathVariable
     ...
     * getHighlightedFlows
     * 
     * code logic: 1. Loop all activities by id asc order; 2. Check each activity's outgoing transitions and
     * eventBoundery outgoing transitions, if outgoing transitions's destination.id is in other executed activityIds,
     * add this transition to highLightedFlows List; 3. But if activity is not a parallelGateway or inclusiveGateway,
        ...
        LinkedList<HistoricActivityInstance> hisActInstList, List<String> highLightedFlows)
    {
        ...
        // ACT-1728 If the process instance was started by a callActivity, it will be not have the startEvent activity
        // in ACT_HI_ACTINST table
        // Code logic:
        ...
        while (!hisActInstList.isEmpty())
        {
            ...
            if (activity != null)
            {
                ...
                List<String> activityHighLightedFlowIds =
                    getHighlightedFlows(allOutgoingTrans, hisActInstList, isParallel);
