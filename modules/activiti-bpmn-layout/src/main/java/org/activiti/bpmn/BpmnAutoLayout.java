...
public class BpmnAutoLayout
{
    ...
    protected void layout(FlowElementsContainer flowElementsContainer)
    {
        ...
        sequenceFlows = new HashMap<String, SequenceFlow>(); // Sequence flow are gathered and processed afterwards,
                                                             // because we must be sure we alreadt found source and
                                                             // target
        boundaryEvents = new ArrayList<BoundaryEvent>(); // Boundary events are gathered and processed afterwards,
                                                         // because we must be sure we have its parent
    ...
    protected void handleSequenceFlow()
    {
        ...
        for (SequenceFlow sequenceFlow : sequenceFlows.values())
        {
            ...
            Object sequenceFlowEdge =
                graph.insertEdge(cellParent, sequenceFlow.getId(), "", sourceVertex, targertVertex, style);
    ...
    protected void createEventVertex(FlowElement flowElement)
    {
        ...
        Object eventVertex =
            graph.insertVertex(cellParent, flowElement.getId(), "", 0, 0, eventSize, eventSize, STYLE_EVENT);
    ...
    protected void createGatewayVertex(FlowElement flowElement)
    {
        ...
        Object gatewayVertex =
            graph.insertVertex(cellParent, flowElement.getId(), "", 0, 0, gatewaySize, gatewaySize, STYLE_GATEWAY);
    ...
    protected void generateSequenceFlowDiagramInterchangeElements()
    {
        for (String sequenceFlowId : generatedEdges.keySet())
        {
            ...
            if (sourceElement instanceof Gateway && ((Gateway)sourceElement).getOutgoingFlows().size() > 1)
            {
                ...
                mxPoint southPoint =
                    new mxPoint(gatewayState.getX() + (gatewayState.getWidth()) / 2, gatewayState.getY()
                        + gatewayState.getHeight());
                mxPoint eastPoint =
                    new mxPoint(gatewayState.getX() + gatewayState.getWidth(), gatewayState.getY()
                        + (gatewayState.getHeight()) / 2);
                mxPoint westPoint =
            ...
            createDiagramInterchangeInformation((SequenceFlow)handledFlowElements.get(sequenceFlowId),
        ...
        for (int i = 0; i < unoptimizedPointsList.size(); i++)
        {
            ...
            if (i > 0 && i != unoptimizedPointsList.size() - 1)
            {
                ...
                if (currentPoint.getX() >= previousPoint.getX() && currentPoint.getX() <= nextPoint.getX()
                    && currentPoint.getY() == previousPoint.getY() && currentPoint.getY() == nextPoint.getY())
                {
                    keepPoint = false;
                }
                else if (currentPoint.getY() >= previousPoint.getY() && currentPoint.getY() <= nextPoint.getY()
                    && currentPoint.getX() == previousPoint.getX() && currentPoint.getX() == nextPoint.getX())
                {
                    keepPoint = false;
                }
    ...
    protected GraphicInfo createDiagramInterchangeInformation(FlowElement flowElement, int x, int y, int width,
        int height)
    {
    ...
    /**
     * Since subprocesses are autolayouted independently (see {@link #handleSubProcess(FlowElement)}), the elements have
     * x and y coordinates relative to the bounds of the subprocess (thinking the subprocess is on (0,0). This however,
     * does not work for nested subprocesses, as they need to take in account the x and y coordinates for each of the
     * parent subproceses.
     * 
