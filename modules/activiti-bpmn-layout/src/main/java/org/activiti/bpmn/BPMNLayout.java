...
public class BPMNLayout extends mxGraphLayout
{
    ...
    {
        return super.isVertexIgnored(vertex) || graph.isSwimlane(vertex)
            || graph.getModel().getGeometry(vertex).isRelative() || graph.getConnections(vertex).length == 0;
    }
    ...
    protected mxRectangle apply(TreeNode node, mxRectangle bounds)
    {
        ...
        if (node.cell != null && g != null)
        {
            ...
            {
                bounds =
                    new mxRectangle(Math.min(bounds.getX(), g.getX()), Math.min(bounds.getY(), g.getY()),
                        Math.max(bounds.getX() + bounds.getWidth(), g.getX() + g.getWidth()), Math.max(bounds.getY()
