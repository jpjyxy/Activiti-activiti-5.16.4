...
public class ActivitiProducer extends DefaultProducer
{
    ...
    private void signal(Exchange exchange)
    {
        ...
        while (firstTime || (timeout > 0 && (System.currentTimeMillis() - initialTime < timeout)))
        {
            ...
            {
                throw new RuntimeException("error occured while waiting for activiti=" + activity
    ...
    private String findProcessInstanceId(Exchange exchange)
    {
        ...
        ProcessInstance processInstance =
            runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(processInstanceKey).singleResult();
        ...
        try
        {
            ...
            if (key == null)
            {
                return runtimeService.startProcessInstanceByKey(processKey,
                    ExchangeUtils.prepareVariables(exchange, endpoint));
            }
            else
            {
                return runtimeService.startProcessInstanceByKey(processKey,
