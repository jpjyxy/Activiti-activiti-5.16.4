...
public class ExchangeUtils
{
     ...
     * 
     * This method will copy the Camel body to the "camelBody" variable. It will copy the Camel body to individual
     * variables within Activiti if it is of type Map&lt;String, Object&gt; or it will copy the Object as it comes.
     * <ul>
     * <li>If the copyVariablesFromProperties parameter is set on the endpoint, the properties are copied instead</li>
     * <li>If the copyCamelBodyToBodyAsString parameter is set on the endpoint, the camelBody is converted to
     * java.lang.String and added as a camelBody variable, unless it is a Map&lt;String, Object&gt;</li>
     * <li>If the copyVariablesFromHeader parameter is set on the endpoint, each Camel Header will be copied to an
     * individual variable within Activiti.</li>
     * </ul>
     * 
            ...
            if (activitiEndpoint.isCopyVariablesFromHeader())
            {
                ...
                for (Map.Entry<String, Object> header : exchange.getIn().getHeaders().entrySet())
                {
                    ...
                    if (isSetProcessInitiator
                        && activitiEndpoint.getProcessInitiatorHeaderName().equals(header.getKey()))
                    {
                        continue;
                    }
    ...
    /**
     * Gets the value of the Camel header that contains the userId to be set as the process initiator. Returns null if
     * no header name was specified on the Camel route.
     * 
     * @param exchange The Camel Exchange object
     * @param activitiEndpoint The ActivitiEndpoint implementation
    ...
    public static String prepareInitiator(Exchange exchange, ActivitiEndpoint activitiEndpoint)
    {
        ...
        if (activitiEndpoint.isSetProcessInitiator())
        {
            ...
            {
                throw new ActivitiException("Initiator header '" + activitiEndpoint.getProcessInitiatorHeaderName()
                    + "': Value must be of type String.", e);
            }
            
            if (!StringUtils.hasText(initiator))
            {
                throw new RuntimeException("Initiator header '" + activitiEndpoint.getProcessInitiatorHeaderName()
