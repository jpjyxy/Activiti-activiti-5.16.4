...
/**
 * This abstract class takes the place of the now-deprecated CamelBehaviour class (which can still be used for legacy
 * compatibility) and significantly improves on its flexibility. Additional implementations can be created that change
 * the way in which Activiti interacts with Camel per your specific needs.
 * 
 * Three out-of-the-box implementations of CamelBehavior are provided: (1) CamelBehaviorDefaultImpl: Works just like
 * CamelBehaviour does; copies variables into and out of Camel as or from properties. (2) CamelBehaviorBodyAsMapImpl:
 * Works by copying variables into and out of Camel using a Map<String,Object> object in the body. (3)
 * CamelBehaviorCamelBodyImpl: Works by copying a single variable value into Camel as a String body and copying the
 * Camel body into that same Activiti variable. The variable in Activiti must be named "camelBody".
 * 
 * The chosen implementation should be set within your ProcessEngineConfiguration. To specify the implementation using
 * Spring, include the following line in your configuration file as part of the properties for
 ...
 * properties to the Activiti variable set; they can safely be ignored, of course, if not required. It will
 * conditionally copy the Camel body to the "camelBody" variable if it is of type java.lang.String, OR it will copy the
 * Camel body to individual variables within Activiti if it is of type Map<String,Object>.
 * 
