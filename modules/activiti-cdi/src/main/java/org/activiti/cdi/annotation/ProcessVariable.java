...
/**
 * Annotation for qualifying injection points such that process variables are injected. Requires a process instance to
 * be managed
 * <ul>
 * <li>{@code @Inject @ProcessVariable Object accountNumber}</li>
 * <li>{@code @Inject @ProcessVariable("accountNumber") Object account}</li>
 * </ul>
 * In both cases, the process variable with the name 'accountNumber' is injected. NOTE: injection points must be of type
 * 'object'.
 * <p />
 * 
 * Can also be used to declare bean-properties to hold process variables in combination with the {@link StartProcess}
 * annotation:
 * 
 * <pre>
