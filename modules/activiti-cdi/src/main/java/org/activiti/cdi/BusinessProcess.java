...
/**
 * Bean supporting contextual business process management. This allows us to implement a unit of work, in which a
 * particular CDI scope (Conversation / Request / Thread) is associated with a particular Execution / ProcessInstance or
 * Task.
 * <p />
 * The protocol is that we <em>associate</em> the {@link BusinessProcess} bean with a particular Execution / Task, then
 * perform some changes (retrieve / set process variables) and then end the unit of work. This bean makes sure that our
 * changes are only "flushed" to the process engine when we successfully complete the unit of work.
 * <p />
 ...
 * </pre>
 * <p />
 * <strong>NOTE:</strong> in the absence of a conversation, (non faces request, i.e. when processing a JAX-RS, JAX-WS,
 * JMS, remote EJB or plain Servlet requests), the {@link BusinessProcess} bean associates with the current Request (see
 * {@link RequestScoped @RequestScoped}).
 * <p />
 * <strong>NOTE:</strong> in the absence of a request, ie. when the activiti JobExecutor accesses
 * {@link BusinessProcessScoped @BusinessProcessScoped} beans, the execution is associated with the current thread.
 * 
 * @author Daniel Meyer
 * @author Falko Menge
 */
...
public class BusinessProcess implements Serializable
{
    ...
    protected void validateValidUsage()
    {
        ...
        {
            throw new ActivitiCdiException(
                "Cannot use this method of the BusinessProcess bean within an activiti command.");
        }
    }
    ...
    public ProcessInstance startProcessById(String processDefinitionId, Map<String, Object> variables)
    {
        ...
        ProcessInstance instance =
            processEngine.getRuntimeService().startProcessInstanceById(processDefinitionId, cachedVariables);
        if (!instance.isEnded())
        {
            setExecution(instance);
        }
    ...
    public ProcessInstance startProcessByKey(String key)
    {
        ...
        ProcessInstance instance =
            processEngine.getRuntimeService().startProcessInstanceByKey(key, getAndClearCachedVariables());
        if (!instance.isEnded())
        {
            setExecution(instance);
        }
    ...
    public ProcessInstance startProcessByKey(String key, String businessKey)
    {
        ...
        ProcessInstance instance =
            processEngine.getRuntimeService().startProcessInstanceByKey(key, businessKey, getAndClearCachedVariables());
        if (!instance.isEnded())
        {
            setExecution(instance);
        }
    ...
    public ProcessInstance startProcessByKey(String key, String businessKey, Map<String, Object> variables)
    {
        ...
        cachedVariables.putAll(variables);
        ProcessInstance instance =
            processEngine.getRuntimeService().startProcessInstanceByKey(key, businessKey, cachedVariables);
        if (!instance.isEnded())
        {
            setExecution(instance);
        }
    ...
    public ProcessInstance startProcessByMessage(String messageName)
    {
        ...
        ProcessInstance processInstance =
            processEngine.getRuntimeService().startProcessInstanceByMessage(messageName, cachedVariables);
    ...
    public ProcessInstance startProcessByMessage(String messageName, String businessKey,
        Map<String, Object> processVariables)
    {
        ...
        cachedVariables.putAll(processVariables);
        ProcessInstance processInstance =
            processEngine.getRuntimeService().startProcessInstanceByMessage(messageName, businessKey, cachedVariables);
        if (!processInstance.isEnded())
        {
            setExecution(processInstance);
        }
    ...
    public ProcessInstance startProcessByName(String string)
    {
        ...
        if (definition == null)
        {
            throw new ActivitiObjectNotFoundException("No process definition found for name: " + string,
                ProcessDefinition.class);
        }
        ProcessInstance instance =
            processEngine.getRuntimeService()
                .startProcessInstanceById(definition.getId(), getAndClearCachedVariables());
        if (!instance.isEnded())
        {
            setExecution(instance);
        }
    ...
    public ProcessInstance startProcessByName(String string, Map<String, Object> variables)
    {
        ...
        cachedVariables.putAll(variables);
        ProcessInstance instance =
            processEngine.getRuntimeService().startProcessInstanceById(definition.getId(), cachedVariables);
        if (!instance.isEnded())
        {
            setExecution(instance);
        }
    ...
    {
        Execution execution =
            processEngine.getRuntimeService().createExecutionQuery().executionId(executionId).singleResult();
        if (execution == null)
        {
            throw new ActivitiCdiException("Cannot associate execution by id: no execution with id '" + executionId
                + "' found.");
        }
     ...
     * <p/>
     * Ends the current unit of work (flushes changes to process variables set using
     * {@link #setVariable(String, Object)} or made on {@link BusinessProcessScoped @BusinessProcessScoped} beans).
     * 
     * @throws ActivitiCdiException if no execution is currently associated
     * @throws ActivitiException if the activiti command fails
     */
     ...
     * 
     * @throws ActivitiCdiException if no such task is found
     */
     ...
     * {@link #setVariable(String, Object)} or made on {@link BusinessProcessScoped @BusinessProcessScoped} beans).
     * 
     * @throws ActivitiCdiException if no task is currently associated
     * @throws ActivitiException if the activiti command fails
     */
     ...
     * <p />
     * 
     * <strong>NOTE:</strong> If no execution is currently associated, the value is temporarily cached and flushed to
     * the process instance at the end of the unit of work
     * 
    ...
    /*
     * Note that Producers should go into {@link CurrentProcessInstance} in order to allow for specializing {@link
     * BusinessProcess}.
     */
     ...
     * 
     * @throws ActivitiCdiException if no {@link Task} is associated. Use {@link #isTaskAssociated()} to check whether
     ...
     * 
     * @throws ActivitiCdiException if no {@link Execution} is associated. Use {@link #isAssociated()} to check whether
     *             an association exists.
     */
    ...
    protected void assertAssociated()
    {
        ...
        {
            throw new ActivitiCdiException(
                "No execution associated. Call busniessProcess.associateExecutionById() or businessProcess.startTask() first.");
        }
    }
