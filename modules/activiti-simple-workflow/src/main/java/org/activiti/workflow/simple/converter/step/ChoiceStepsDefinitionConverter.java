 ...
 */
public class ChoiceStepsDefinitionConverter extends
    BaseStepDefinitionConverter<ChoiceStepsDefinition, ExclusiveGateway>
{
    ...
    protected ExclusiveGateway createProcessArtifact(ChoiceStepsDefinition choiceStepsDefinition,
        ...
        for (ListConditionStepDefinition<ChoiceStepsDefinition> stepListDefinition : choiceStepsDefinition.getStepList())
        {
            ...
            for (int i = 0; i < stepListDefinition.getSteps().size(); i++)
            {
                ...
                FlowElement flowElement =
                    (FlowElement)conversionFactory.getStepConverterFor(step).convertStepDefinition(step, conversion);
                ...
                if (i == 0)
                {
                    ...
                    {
                        conditionBuilder.append("}");
                        SequenceFlow mainFlow =
                            addSequenceFlow(conversion,
                                forkGateway.getId(),
                                flowElement.getId(),
            ...
            {
                // Special case for a "stepless" stepListDefinition, which should just create a sequence-flow from the
                // fork to the join
