 ...
 * 
 * When added to a {@link WorkflowDefinitionConversionFactory}, this class will make sure a start event and end event is
 * created for the {@link Process}. Further, it will generate a correct incoming and outgoing sequence flow list for
 * each {@link Process} element, as required by some toolings (eg Modeler).
 * 
 * @author Joram Barrez
 */
public class DefaultWorkflowDefinitionConversionListener implements WorkflowDefinitionConversionListener
{
    ...
    protected void initializeProcess(WorkflowDefinitionConversion conversion)
    {
        ...
        if (workflowDefinition.getStartFormDefinition() != null
            && workflowDefinition.getStartFormDefinition().getFormKey() != null)
        {
    ...
    public void afterStepsConversion(WorkflowDefinitionConversion conversion)
    {
        ...
        {
            List<SequenceFlow> incomingSequenceFlow =
                sequenceFlowMapping.getIncomingSequenceFlowMapping().get(flowNode.getId());
            ...
            List<SequenceFlow> outgoingSequenceFlow =
                sequenceFlowMapping.getOutgoingSequenceFlowMapping().get(flowNode.getId());
