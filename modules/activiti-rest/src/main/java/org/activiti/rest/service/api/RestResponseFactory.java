 ...
 * 
 * Added a new "createProcessInstanceResponse" method (with a different signature) to conditionally return the process
 * variables that exist within the process instance when the first wait state is encountered (or when the process
 * instance completes). Also added the population of a "completed" flag - within both the original
 * "createProcessInstanceResponse" method and the new one with the different signature - to let the caller know whether
 * the process instance has completed or not.
 * 
...
public class RestResponseFactory
{
    ...
    public TaskResponse createTaskResponse(Task task, String serverRootUrl)
    {
        ...
        {
            response.setProcessDefinitionUrl(formatUrl(serverRootUrl,
                RestUrls.URL_PROCESS_DEFINITION,
        ...
        {
            response.setProcessInstanceUrl(formatUrl(serverRootUrl,
                RestUrls.URL_PROCESS_INSTANCE,
        ...
        if (task.getProcessVariables() != null)
        {
            ...
            {
                response.addVariable(createRestVariable(name,
    ...
    public DeploymentResourceResponse createDeploymentResourceResponse(String deploymentId, String resourceId,
        String contentType, String serverRootUrl)
    {
        // Create URL's
        String resourceUrl = formatUrl(serverRootUrl, RestUrls.URL_DEPLOYMENT_RESOURCE, deploymentId, resourceId);
        String resourceContentUrl =
            formatUrl(serverRootUrl, RestUrls.URL_DEPLOYMENT_RESOURCE_CONTENT, deploymentId, resourceId);
    ...
    public ProcessDefinitionResponse createProcessDefinitionResponse(ProcessDefinition processDefinition,
        boolean graphicalNotationDefined, String serverRootUrl)
    {
        ...
        response.setResource(formatUrl(serverRootUrl,
            RestUrls.URL_DEPLOYMENT_RESOURCE,
            processDefinition.getDeploymentId(),
            processDefinition.getResourceName()));
        if (processDefinition.getDiagramResourceName() != null)
        {
            response.setDiagramResource(formatUrl(serverRootUrl,
                RestUrls.URL_DEPLOYMENT_RESOURCE,
                processDefinition.getDeploymentId(),
    ...
    public List<RestVariable> createRestVariables(Map<String, Object> variables, String id, int variableType,
        RestVariableScope scope, String serverRootUrl)
    {
    ...
    public RestVariable createRestVariable(String name, Object value, RestVariableScope scope, String id,
        int variableType, boolean includeBinaryValue, String serverRootUrl)
    {
        ...
        if (value != null)
        {
            ...
            else
            {
                ...
                {
                    restVar.setValueUrl(formatUrl(serverRootUrl,
                        RestUrls.URL_HISTORIC_TASK_INSTANCE_VARIABLE_DATA,
                        id,
                        name));
                }
                ...
                {
                    restVar.setValueUrl(formatUrl(serverRootUrl,
                        RestUrls.URL_HISTORIC_PROCESS_INSTANCE_VARIABLE_DATA,
                        id,
                        name));
                }
        ...
        String executionId, String processInstanceId, String serverRootUrl)
    {
        ...
        {
            restVar.setValueUrl(formatUrl(serverRootUrl,
                RestUrls.URL_PROCESS_INSTANCE_VARIABLE_DATA,
                processInstanceId,
                name));
        }
    ...
    {
        return createRestIdentityLink(link.getType(),
            link.getUserId(),
            link.getGroupId(),
            link.getTaskId(),
    ...
    public RestIdentityLink createRestIdentityLink(String type, String userId, String groupId, String taskId,
        String processDefinitionId, String processInstanceId, String serverRootUrl)
    {
        ...
        {
            result.setUrl(formatUrl(serverRootUrl,
                RestUrls.URL_PROCESS_DEFINITION_IDENTITYLINK,
                processDefinitionId,
                family,
        ...
        {
            result.setUrl(formatUrl(serverRootUrl,
                RestUrls.URL_TASK_IDENTITYLINK,
                taskId,
                family,
                (userId != null ? userId : groupId),
                type));
        ...
        {
            result.setUrl(formatUrl(serverRootUrl,
                RestUrls.URL_PROCESS_INSTANCE_IDENTITYLINK,
                processInstanceId,
                (userId != null ? userId : groupId),
                type));
        }
    ...
    public CommentResponse createRestComment(Comment comment, String serverRootUrl)
    {
        ...
        {
            result.setProcessInstanceUrl(formatUrl(serverRootUrl,
                RestUrls.URL_HISTORIC_PROCESS_INSTANCE_COMMENT,
    ...
    public AttachmentResponse createAttachmentResponse(Attachment attachment, String serverRootUrl)
    {
        ...
        if (attachment.getUrl() == null && attachment.getTaskId() != null)
        {
            ...
            result.setContentUrl(formatUrl(serverRootUrl,
                RestUrls.URL_TASK_ATTACHMENT_DATA,
    ...
    public ProcessInstanceResponse createProcessInstanceResponse(ProcessInstance processInstance, String serverRootUrl)
    {
        ...
        result.setProcessDefinitionUrl(formatUrl(serverRootUrl,
            RestUrls.URL_PROCESS_DEFINITION,
        ...
        if (processInstance.getProcessVariables() != null)
        {
            ...
            for (String name : variableMap.keySet())
            {
                result.addVariable(createRestVariable(name,
                    variableMap.get(name),
                    RestVariableScope.LOCAL,
    ...
    public ProcessInstanceResponse createProcessInstanceResponse(ProcessInstance processInstance,
        boolean returnVariables, Map<String, Object> runtimeVariableMap,
        List<HistoricVariableInstance> historicVariableList, String serverRootUrl)
    {
    ...
    public FormDataResponse createFormDataResponse(FormData formData, String serverRootUrl)
    {
        ...
        if (formData instanceof StartFormData)
        {
            ...
            if (startFormData.getProcessDefinition() != null)
            {
                ...
                result.setProcessDefinitionUrl(formatUrl(serverRootUrl,
                    RestUrls.URL_PROCESS_DEFINITION,
                    startFormData.getProcessDefinition().getId()));
            }
        }
    ...
    public HistoricProcessInstanceResponse createHistoricProcessInstanceResponse(
        HistoricProcessInstance processInstance, String serverRootUrl)
    {
        ...
        String serverRootUrl)
    {
        ...
        {
            result.setProcessInstanceUrl(formatUrl(serverRootUrl,
                RestUrls.URL_HISTORIC_PROCESS_INSTANCE,
    ...
    public HistoricActivityInstanceResponse createHistoricActivityInstanceResponse(
        HistoricActivityInstance activityInstance, String serverRootUrl)
    {
    ...
    public HistoricVariableInstanceResponse createHistoricVariableInstanceResponse(
        HistoricVariableInstance variableInstance, String serverRootUrl)
    {
        ...
        result.setVariable(createRestVariable(variableInstance.getVariableName(),
    ...
    public HistoricDetailResponse createHistoricDetailResponse(HistoricDetail detail, String serverRootUrl)
    {
        ...
        else if (detail instanceof HistoricVariableUpdate)
        {
            ...
            result.setVariable(createRestVariable(variableUpdate.getVariableName(),
    ...
    /**
     * @return list of {@link RestVariableConverter} which are used by this factory. Additional converters can be added
     *         and existing ones replaced ore removed.
     */
    ...
    /**
     * Called once when the converters need to be initialized. Override of custom conversion needs to be done between
     * java and rest.
     */
