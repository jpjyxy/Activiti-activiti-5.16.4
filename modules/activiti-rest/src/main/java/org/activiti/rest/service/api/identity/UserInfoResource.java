...
public class UserInfoResource extends BaseUserResource
{
    ...
    String userId, @PathVariable("key")
    String key, HttpServletRequest request)
    {
        ...
        {
            throw new ActivitiObjectNotFoundException("User info with key '" + key + "' does not exists for user '"
                + user.getId() + "'.", null);
        }
    ...
    UserInfoRequest userRequest, HttpServletRequest request)
    {
