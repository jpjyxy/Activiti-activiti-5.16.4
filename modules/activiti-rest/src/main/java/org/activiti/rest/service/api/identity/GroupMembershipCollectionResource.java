...
public class GroupMembershipCollectionResource extends BaseGroupResource
{
    ...
    public MembershipResponse createMembership(@PathVariable
    String groupId, @RequestBody
    MembershipRequest memberShip, HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (identityService.createUserQuery().memberOfGroup(group.getId()).userId(memberShip.getUserId()).count() > 0)
        {
            ...
            throw new ActivitiConflictException("User '" + memberShip.getUserId() + "' is already part of group '"
                + group.getId() + "'.");
        }
