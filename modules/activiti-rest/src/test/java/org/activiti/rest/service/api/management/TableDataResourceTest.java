...
public class TableDataResourceTest extends BaseSpringRestTestCase
{
        ...
        try
        {
            ...
            HttpResponse response =
                executeHttpRequest(new HttpGet(SERVER_URL_PREFIX
                    + RestUrls.createRelativeResourceUrl(RestUrls.URL_TABLE_DATA, tableName)),
                    HttpStatus.SC_OK);
            ...
            response =
                executeHttpRequest(new HttpGet(SERVER_URL_PREFIX
                    + RestUrls.createRelativeResourceUrl(RestUrls.URL_TABLE_DATA, tableName)
                    + "?orderAscendingColumn=LONG_&start=1&size=1"),
                    HttpStatus.SC_OK);
    ...
    public void testGetDataForUnexistingTable()
        throws Exception
    {
        executeHttpRequest(new HttpGet(SERVER_URL_PREFIX
            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TABLE_DATA, "unexisting")),
            HttpStatus.SC_NOT_FOUND);
    }
    
    public void testGetDataSortByIllegalColumn()
        throws Exception
    {
        // We use variable-table as a reference
        String tableName = managementService.getTableName(VariableInstanceEntity.class);
        executeHttpRequest(new HttpGet(SERVER_URL_PREFIX
            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TABLE_DATA, tableName)
            + "?orderAscendingColumn=unexistingColumn"),
