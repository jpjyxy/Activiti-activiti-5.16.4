...
public class TaskIdentityLinkResourceTest extends BaseSpringRestTestCase
{
        ...
        for (int i = 0; i < responseNode.size(); i++)
        {
            ...
            if (!link.get("user").isNull())
            {
                if (link.get("user").textValue().equals("john"))
                {
                    ...
                    assertTrue(link.get("url")
                        .textValue()
                        .endsWith(RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_IDENTITYLINK,
                            task.getId(),
                            "users",
                            "john",
                            "customType")));
            ...
            else if (!link.get("group").isNull())
            {
                ...
                assertTrue(link.get("url")
                    ...
                    .endsWith(RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_IDENTITYLINK,
                        task.getId(),
                        RestUrls.SEGMENT_IDENTITYLINKS_FAMILY_GROUPS,
                        "sales",
                        "candidate")));
     ...
     */
    public void testCreateIdentityLink()
        throws Exception
    {
        try
        {
            ...
            assertTrue(responseNode.get("url")
                ...
                .endsWith(RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_IDENTITYLINK,
                    task.getId(),
                    RestUrls.SEGMENT_IDENTITYLINKS_FAMILY_USERS,
                    "kermit",
                    "myType")));
     ...
     */
    public void testGetSingleIdentityLink()
        throws Exception
    {
        try
        {
            ...
            HttpGet httpGet =
                new HttpGet(SERVER_URL_PREFIX
                    + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_IDENTITYLINK,
                        task.getId(),
                        "users",
                        "kermit",
                        "myType"));
     ...
     * runtime/tasks/{taskId}/identitylinks/{family}/{identityId}/{type}
     */
    public void testDeleteSingleIdentityLink()
        throws Exception
    {
        try
        {
