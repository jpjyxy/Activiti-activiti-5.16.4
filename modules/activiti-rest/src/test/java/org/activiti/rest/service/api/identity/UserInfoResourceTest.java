...
public class UserInfoResourceTest extends BaseSpringRestTestCase
{
     ...
     */
    public void testGetUserInfoCollection()
        throws Exception
    {
        User savedUser = null;
        try
        {
            ...
            HttpResponse response =
                executeHttpRequest(new HttpGet(SERVER_URL_PREFIX
                    + RestUrls.createRelativeResourceUrl(RestUrls.URL_USER_INFO_COLLECTION, newUser.getId())),
                    HttpStatus.SC_OK);
            ...
            for (int i = 0; i < responseNode.size(); i++)
            {
                ...
                else if (info.get("key").textValue().equals("key2"))
                {
                    assertTrue(info.get("url")
                        .textValue()
                        .endsWith(RestUrls.createRelativeResourceUrl(RestUrls.URL_USER_INFO, newUser.getId(), "key2")));
                    foundSecond = true;
                }
            }
     ...
     */
    public void testGetInfoForUnexistingUser()
        throws Exception
    {
        executeHttpRequest(new HttpGet(SERVER_URL_PREFIX
     ...
     */
    public void testGetInfoForUserWithoutInfo()
        throws Exception
    {
        User savedUser = null;
        try
     ...
     */
    public void testUpdateInfoForUnexistingUser()
        throws Exception
    {
        ObjectNode requestNode = objectMapper.createObjectNode();
        requestNode.put("value", "Updated value");
     ...
     */
    public void testUpdateUnexistingInfo()
        throws Exception
    {
        User savedUser = null;
        try
     ...
     */
    public void testDeleteInfoForUnexistingUser()
        throws Exception
    {
        executeHttpRequest(new HttpDelete(SERVER_URL_PREFIX
            + RestUrls.createRelativeResourceUrl(RestUrls.URL_USER_INFO, "unexisting", "key1")),
     ...
     * Test deleting the info for a user who doesn't have that info set
     */
    public void testDeleteInfoForUserWithoutInfo()
        throws Exception
    {
        User savedUser = null;
        try
    ...
    public void testCreateUserInfoExceptions()
        throws Exception
    {
        User savedUser = null;
        try
