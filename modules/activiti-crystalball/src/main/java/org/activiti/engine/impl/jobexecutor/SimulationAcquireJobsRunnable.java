...
/**
 * "thread" has to be driven by simulation time. In fact new thread is not started. Simulation is driven by simulation
 * time. In case of thread for acquiring jobs, synchronization will be over complicated.
 * 
    ...
    public synchronized void run()
    {
        ...
        try
        {
            ...
            if (jobsAcquired < maxJobsPerAcquisition)
            {
                ...
                List<TimerEntity> nextTimers =
                    commandExecutor.execute(new GetUnlockedTimersByDuedateCmd(duedate, new Page(0, 1)));
                
                if (!nextTimers.isEmpty())
                {
                    long millisTillNextTimer =
                        nextTimers.get(0).getDuedate().getTime()
                            - SimulationRunContext.getClock().getCurrentTime().getTime();
                ...
                synchronized (MONITOR)
                {
                    if (!isInterrupted)
                    {
                        ...
                        SimulationEvent event =
                            new SimulationEvent.Builder(SimulationEvent.TYPE_ACQUIRE_JOB_NOTIFICATION_EVENT).simulationTime(SimulationRunContext.getClock()
                                .getCurrentTime()
                                .getTime()
                                + millisToWait)
                                .property(this)
