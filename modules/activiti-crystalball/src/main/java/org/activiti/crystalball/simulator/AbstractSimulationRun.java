...
public abstract class AbstractSimulationRun implements SimulationRun, SimulationDebugger
{
    ...
    protected SimulationEvent removeSimulationEvent()
    {
        ...
        if (event != null && event.hasSimulationTime())
            this.processEngine.getProcessEngineConfiguration()
                .getClock()
                .setCurrentTime(new Date(event.getSimulationTime()));
        return event;
    }
    ...
    {
        SimulationEvent breakEvent =
            new SimulationEvent.Builder(SimulationEvent.TYPE_BREAK_SIMULATION).simulationTime(simulationTime)
                .priority(SimulationEvent.PRIORITY_SYSTEM)
                .build();
