...
public class SimpleSimulationRun extends AbstractSimulationRun
{
    ...
    protected SimpleSimulationRun(Builder builder)
    {
        ...
        if (builder.getJobExecutor() != null)
            eventHandlerMap.put(SimulationEvent.TYPE_ACQUIRE_JOB_NOTIFICATION_EVENT,
                new AcquireJobNotificationEventHandler(builder.getJobExecutor()));
    }
    ...
    {// init new process engine
        try
        {
            ...
            if (dueDate != null)
                SimulationRunContext.getEventCalendar().addEvent(new SimulationEvent.Builder(
                    SimulationEvent.TYPE_END_SIMULATION).simulationTime(dueDate.getTime()).build());
        }
        catch (Exception e)
        {
