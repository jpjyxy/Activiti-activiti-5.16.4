...
public class BoundaryEventValidator extends ProcessLevelValidator
{
    ...
    protected void executeValidation(BpmnModel bpmnModel, Process process, List<ValidationError> errors)
    {
        ...
        // Only one boundary event of type 'cancel' can be attached to the same element, so we store the count
        // temporarily here
        HashMap<String, Integer> cancelBoundaryEventsCounts = new HashMap<String, Integer>();
        
        // Only one boundary event of type 'compensate' can be attached to the same element, so we store the count
        // temporarily here
            ...
            if (boundaryEvent.getEventDefinitions() != null && !boundaryEvent.getEventDefinitions().isEmpty())
            {
                ...
                if (eventDefinition instanceof CancelEventDefinition)
                {
                    ...
                    else
                    {
                        ...
                        cancelBoundaryEventsCounts.put(attachedToFlowElement.getId(), new Integer(
