...
/**
 * A {@link ProcessEngineConfigurator} that integrates a LDAP system with the Activiti process engine. The LDAP system
 * will be consulted primarily for getting user information and in particular for fetching groups of a user.
 * 
...
public class LDAPConfigurator extends AbstractProcessEngineConfigurator
{
    ...
    public void configure(ProcessEngineConfigurationImpl processEngineConfiguration)
    {
        ...
        processEngineConfiguration.getSessionFactories().put(ldapUserManagerFactory.getSessionType(),
            ldapUserManagerFactory);
        
        LDAPGroupManagerFactory ldapGroupManagerFactory = getLdapGroupManagerFactory(clock);
        processEngineConfiguration.getSessionFactories().put(ldapGroupManagerFactory.getSessionType(),
    ...
    /**
     * Allows to set all LDAP connection parameters which do not have a dedicated setter. See for example
     * http://docs.oracle.com/javase/tutorial/jndi/ldap/jndi.html for custom properties. Such properties are for example
     * to configure connection pooling, specific security settings, etc.
     * 
     * All the provided parameters will be provided when creating a {@link InitialDirContext}, ie when a connection to
     * the LDAP system is established.
     */
     ...
     * 
     * Use {@link #setUserBaseDn(String)} or {@link #setGroupBaseDn(String)} when needing to differentiate between user
     * and group base DN.
     */
     ...
     * 
     * Here, all the objects in LDAP with the class 'inetOrgPerson' and who have the matching 'uid' attribute value will
     * be returned.
     * 
     * As shown in the example, the user id is injected by the typical {@link MessageFormat}, ie by using <i>{0}</i>
     * 
     * If setting the query alone is insufficient for your specific LDAP setup, you can alternatively plug in a
     * different {@link LDAPQueryBuilder}, which allows for more customization than only the query.
     */
     ...
     * 
     * Here, all the objects in LDAP with the class 'inetOrgPerson' and who have the matching first name or last name
     * will be returned
     * 
     * Several things will be injected in the expression: {0} : the first name attribute {1} : the search text {2} : the
     * last name attribute {3} : the search text
     * 
     * If setting the query alone is insufficient for your specific LDAP setup, you can alternatively plug in a
     * different {@link LDAPQueryBuilder}, which allows for more customization than only the query.
     */
     ...
     * 
     * Here, all the objects in LDAP with the class 'groupOfUniqueNames' and where the provided DN is a 'uniqueMember'
     * are returned.
     * 
     * As shown in the example, the user id is injected by the typical {@link MessageFormat}, ie by using <i>{0}</i>
     * 
     ...
     * 
     * This property is used when looking for a {@link User} object and the mapping between the LDAP object and the
     * Activiti {@link User} object is done.
     * 
     ...
     * 
     * This property is used when looking for a {@link Group} object and the mapping between the LDAP object and the
     * Activiti {@link Group} object is done.
     */
    ...
    /**
     * Set a custom implementation of the {@link LDAPMembershipManagerFactory} if the default implementation is not
     * suitable.
     */
    ...
    /**
     * Set a custom {@link LDAPQueryBuilder} if the default implementation is not suitable. The {@link LDAPQueryBuilder}
     * instance is used when the {@link LDAPUserManager} or {@link LDAPGroupManager} does an actual query against the
     * LDAP system.
     * 
     * The default implementation uses the properties as set on this instance such as
     * {@link #setQueryGroupsForUser(String)} and {@link #setQueryUserByUserId(String)}.
     */
    ...
    /**
     * Allows to set the size of the {@link LDAPGroupCache}. This is an LRU cache that caches groups for users and thus
     * avoids hitting the LDAP system each time the groups of a user needs to be known.
     * 
     * The cache will not be instantiated if the value is less then zero. By default set to -1, so no caching is done.
     * 
     * Note that the group cache is instantiated on the {@link LDAPGroupManagerFactory}. As such, if you have a custom
     * implementation of the {@link LDAPGroupManagerFactory}, do not forget to add the group cache functionality.
     */
    ...
    /**
     * Sets the expiration time of the {@link LDAPGroupCache} in milliseconds. When groups for a specific user are
     * fetched, and if the group cache exists (see {@link #setGroupCacheSize(int)}), the groups will be stored in this
     * cache for the time set in this property. ie. when the groups were fetched at 00:00 and the expiration time is 30
     * mins, any fetch of the groups for that user after 00:30 will not come from the cache, but do a fetch again from
     * the LDAP system. Likewise, everything group fetch for that user done between 00:00 - 00:30 will come from the
     * cache.
     * 
