 ...
 * 
 * Note that only a few methods are actually implemented, as many of the operations (save, update, etc.) are done on the
 * LDAP system directly.
 * 
 * @author Joram Barrez
 */
public class LDAPUserManager extends AbstractManager implements UserIdentityManager
{
            ...
            public UserEntity executeInContext(InitialDirContext initialDirContext)
            {
                try
                {
                    ...
                    String searchExpression =
                        ldapConfigurator.getLdapQueryBuilder().buildQueryByUserId(ldapConfigurator, userId);
                    
                    String baseDn =
                        ldapConfigurator.getUserBaseDn() != null ? ldapConfigurator.getUserBaseDn()
                            : ldapConfigurator.getBaseDn();
                    NamingEnumeration<?> namingEnum =
                        initialDirContext.search(baseDn, searchExpression, createSearchControls());
        ...
        else if (query.getFullNameLike() != null)
        {
            ...
            return ldapTemplate.execute(new LDAPCallBack<List<User>>()
            {
                ...
                public List<User> executeInContext(InitialDirContext initialDirContext)
                {
                    ...
                    {
                        String searchExpression =
                            ldapConfigurator.getLdapQueryBuilder().buildQueryByFullNameLike(ldapConfigurator,
                                query.getFullNameLike());
                        String baseDn =
                            ldapConfigurator.getUserBaseDn() != null ? ldapConfigurator.getUserBaseDn()
                                : ldapConfigurator.getBaseDn();
                        NamingEnumeration<?> namingEnum =
    ...
    protected void mapSearchResultToUser(SearchResult result, UserEntity user)
        throws NamingException
    {
        ...
        if (ldapConfigurator.getUserFirstNameAttribute() != null)
        {
            try
            {
                user.setFirstName(result.getAttributes()
                    .get(ldapConfigurator.getUserFirstNameAttribute())
                    .get()
                    .toString());
            }
        ...
        if (ldapConfigurator.getUserLastNameAttribute() != null)
        {
            try
            {
                user.setLastName(result.getAttributes()
                    .get(ldapConfigurator.getUserLastNameAttribute())
                    .get()
                    .toString());
            }
            catch (NullPointerException e)
            ...
            return ldapTemplate.execute(new LDAPCallBack<Boolean>()
            {
                ...
                public Boolean executeInContext(InitialDirContext initialDirContext)
                {
                    ...
                    if (userDn != null)
                    {
                        ...
                        {
                            verificationContext =
                                LDAPConnectionUtil.createDirectoryContext(ldapConfigurator, userDn, password);
                        }
