...
public abstract class BaseBpmnJsonConverter implements EditorJsonConstants, StencilConstants
{
        ...
        ArrayNode shapesArrayNode, double subProcessX, double subProcessY)
    {
        ...
        flowElementNode =
            BpmnJsonConverterUtil.createChildShape(flowElement.getId(),
                stencilId,
        ...
        BaseElement parentElement, Map<String, JsonNode> shapeMap)
    {
        ...
        if (flowElement instanceof Activity)
        {
                ...
                || StringUtils.isNotEmpty(multiInstanceCondition) || StringUtils.isNotEmpty(multiInstanceLoopType))
            {
                ...
                multiInstanceObject.setSequential(getPropertyValueAsBoolean(PROPERTY_MULTIINSTANCE_SEQUENTIAL,
                    elementNode));
                
                // There is another property used for multi-instance sequential control, that is used for rendering the
                // correct
    ...
    protected void addFormProperties(List<FormProperty> formProperties, ObjectNode propertiesNode)
    {
        ...
        for (FormProperty property : formProperties)
        {
            ...
            propertyItemNode.put(PROPERTY_FORM_WRITEABLE, property.isWriteable() ? PROPERTY_VALUE_YES
                : PROPERTY_VALUE_NO);
            ...
            if (itemsArrayNode != null)
            {
                for (JsonNode formNode : itemsArrayNode)
                {
                    ...
                    if (formIdNode != null && StringUtils.isNotEmpty(formIdNode.asText()))
                    {
                        ...
                        if (formValuesNode != null && StringUtils.isNotEmpty(formValuesNode.asText())
                            && !("undefined".equals(formValuesNode.asText())))
                        {
    ...
    protected void convertJsonToTimerDefinition(JsonNode objectNode, Event event)
    {
        ...
        if (StringUtils.isNotEmpty(timeDate) || StringUtils.isNotEmpty(timeCycle)
            || StringUtils.isNotEmpty(timeDuration))
        {
