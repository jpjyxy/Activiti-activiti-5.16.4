...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor
{
    ...
    public ObjectNode convertToJson(BpmnModel model)
    {
        ...
        if (!model.getPools().isEmpty())
        {
            for (Pool pool : model.getPools())
            {
                ...
                ObjectNode poolNode =
                    BpmnJsonConverterUtil.createChildShape(pool.getId(),
                        STENCIL_POOL,
                ...
                if (process != null)
                {
                    ...
                    processFlowElements(process.findFlowElementsOfType(SequenceFlow.class),
                        model,
                        shapesArrayNode,
                        poolGraphicInfo.getX(),
                        poolGraphicInfo.getY());
                    ...
                    for (Lane lane : process.getLanes())
                    {
                        ...
                        ObjectNode laneNode =
                            BpmnJsonConverterUtil.createChildShape(lane.getId(),
                                STENCIL_LANE,
                        ...
                        for (FlowElement flowElement : process.getFlowElements())
                        {
                            if (lane.getFlowReferences().contains(flowElement.getId()))
                            {
                                ...
                                if (converter != null)
                                {
                                    try
                                    {
                                        converter.newInstance().convertToJson(flowElement,
                                            this,
                                            model,
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode)
    {
        ...
        if (emptyPoolFound)
        {
            ...
            JsonNode processExecutionListenerNode =
                modelNode.get(EDITOR_SHAPE_PROPERTIES).get(PROPERTY_EXECUTION_LISTENERS);
        ...
        if (itemsArrayNode != null)
        {
            for (JsonNode itemNode : itemsArrayNode)
            {
                ...
                if (typeNode != null && StringUtils.isNotEmpty(typeNode.asText()))
                {
                    ...
                    else if (StringUtils.isNotEmpty(itemNode.get(PROPERTY_EXECUTION_LISTENER_DELEGATEEXPRESSION)
                        .asText()))
                    {
                        listener.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_DELEGATEEXPRESSION);
                        listener.setImplementation(itemNode.get(PROPERTY_EXECUTION_LISTENER_DELEGATEEXPRESSION)
                            .asText());
                    }
                        ...
                        if (listenerFieldsArrayNode != null)
                        {
                            for (JsonNode fieldNode : listenerFieldsArrayNode)
                            {
                                ...
                                if (fieldNameNode != null && StringUtils.isNotEmpty(fieldNameNode.asText()))
                                {
                                    ...
                                    field.setStringValue(getValueAsString(PROPERTY_EXECUTION_LISTENER_FIELD_VALUE,
                                        fieldNode));
                                    field.setExpression(getValueAsString(PROPERTY_EXECUTION_LISTENER_EXPRESSION,
                                        fieldNode));
                                    fields.add(field);
                                }
                            }
    ...
    private void postProcessElements(FlowElementsContainer process, Collection<FlowElement> flowElementList)
    {
        ...
        for (FlowElement flowElement : flowElementList)
        {
            ...
            if (flowElement instanceof BoundaryEvent)
            {
                ...
                Activity activity =
                    retrieveAttachedRefObject(boundaryEvent.getAttachedToRefId(), process.getFlowElements());
        ...
        for (String edgeId : edgeMap.keySet())
        {
            ...
            if (DI_CIRCLES.contains(sourceRefStencilId))
            {
                Circle2D eventCircle =
                    new Circle2D(sourceInfo.getX() + sourceDockersX, sourceInfo.getY() + sourceDockersY, sourceDockersX);
            ...
            else if (DI_CIRCLES.contains(targetRefStencilId))
            {
                ...
                Circle2D eventCircle =
                    new Circle2D(targetInfo.getX() + targetDockersX, targetInfo.getY() + targetDockersY, targetDockersX);
    ...
    private Polyline2D createRectangle(GraphicInfo graphicInfo)
    {
        Polyline2D rectangle =
            new Polyline2D(new Point2D(graphicInfo.getX(), graphicInfo.getY()), new Point2D(graphicInfo.getX()
                + graphicInfo.getWidth(), graphicInfo.getY()), new Point2D(graphicInfo.getX() + graphicInfo.getWidth(),
                graphicInfo.getY() + graphicInfo.getHeight()), new Point2D(graphicInfo.getX(), graphicInfo.getY()
                + graphicInfo.getHeight()), new Point2D(graphicInfo.getX(), graphicInfo.getY()));
        return rectangle;
    }
    ...
    private Polyline2D createGateway(GraphicInfo graphicInfo)
    {
        ...
        Polyline2D gatewayRectangle =
            new Polyline2D(new Point2D(graphicInfo.getX(), middleY), new Point2D(middleX, graphicInfo.getY()),
                new Point2D(graphicInfo.getX() + graphicInfo.getWidth(), middleY), new Point2D(middleX,
                    graphicInfo.getY() + graphicInfo.getHeight()), new Point2D(graphicInfo.getX(), middleY));
