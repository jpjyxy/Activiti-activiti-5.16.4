...
public class SuspendedProcessDefinitionPage extends ManagementPage
{
    ...
    {
        ExplorerApp.get().setCurrentUriFragment(new UriFragment(
            SuspendedProcessDefinitionNavigator.SUSPENDED_PROC_DEF_URI_PART));
    }
    ...
    protected Table createList()
    {
        ...
        processDefinitionTable.addListener(new Property.ValueChangeListener()
        {
            ...
            {
                Item item = processDefinitionTable.getItem(event.getProperty().getValue()); // the value of the property
                                                                                            // is the itemId of the
                                                                                            // table entry
                if (item != null)
                {
                    String processDefinitionId = (String)item.getItemProperty("id").getValue();
                    setDetailComponent(new SuspendedProcessDefinitionDetailPanel(processDefinitionId,
                        SuspendedProcessDefinitionPage.this));
                    
                    // Update URL
                    ExplorerApp.get().setCurrentUriFragment(new UriFragment(
                        SuspendedProcessDefinitionNavigator.SUSPENDED_PROC_DEF_URI_PART, processDefinitionId));
