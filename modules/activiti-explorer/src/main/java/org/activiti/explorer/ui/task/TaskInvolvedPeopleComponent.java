...
public class TaskInvolvedPeopleComponent extends CustomComponent
{
    ...
    protected UserDetailsComponent createOwnerComponent()
    {
        ...
        return new UserDetailsComponent(task.getOwner(), i18nManager.getMessage(roleMessage),
            i18nManager.getMessage(Messages.TASK_OWNER_TRANSFER), new ChangeOwnershipListener(task, taskDetailPanel));
    }
    ...
    protected UserDetailsComponent createAssigneeComponent()
    {
        ...
        return new UserDetailsComponent(task.getAssignee(), i18nManager.getMessage(roleMessage),
    ...
    protected void initInvolvedPeople()
    {
        ...
        for (final IdentityLink identityLink : identityLinks)
        {
            ...
            { // only user identity links, ignoring the group ids
                ...
                {
                    UserDetailsComponent involvedDetails =
                        new UserDetailsComponent(identityLink.getUserId(), identityLink.getType(),
                            i18nManager.getMessage(Messages.TASK_INVOLVED_REMOVE), new RemoveInvolvedPersonListener(
                                identityLink, task, taskDetailPanel));
