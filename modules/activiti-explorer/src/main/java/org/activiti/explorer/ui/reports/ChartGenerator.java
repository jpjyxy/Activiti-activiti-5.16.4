...
public class ChartGenerator
{
    ...
    public static ChartComponent generateChart(byte[] reportData)
    {
        ...
        {
            chartComponent.addChart(null,
                null,
                ExplorerApp.get().getI18nManager().getMessage(Messages.REPORTING_ERROR_NOT_ENOUGH_DATA));
        ...
        if (datasetsNode != null && datasetsNode.isArray())
        {
            ...
            while (dataIterator.hasNext())
            {
                ...
                {
                    chartComponent.addChart(description,
                        null,
                        ExplorerApp.get().getI18nManager().getMessage(Messages.REPORTING_ERROR_NOT_ENOUGH_DATA));
                }
                else
    ...
    protected static Component createChart(JsonNode dataNode, String[] names, Number[] values)
    {
        ...
        if (CHART_TYPE_BAR_CHART.equals(type))
        {
            ...
            Axes axes =
                new Axes().addAxis(new XYaxis().setRenderer(AxisRenderers.CATEGORY)
        ...
        else if (CHART_TYPE_LINE_CHART.equals(type))
        {
            ...
            Axes axes =
                new Axes().addAxis(new XYaxis().setLabel(xAxis != null ? xAxis : "")
                    .setMin(names[0])
                    .setMax(names[values.length - 1])
                    .setDrawMajorTickMarks(true)).addAxis(new XYaxis(XYaxes.Y).setLabel(yAxis != null ? yAxis : "")
                    .setDrawMajorTickMarks(true));
            ...
            Series series =
                new Series().addSeries(new XYseries().setShowLine(true)
                    .setMarkerOptions(new MarkerRenderer().setShadow(true).setSize(7).setStyle(MarkerStyles.CIRCLE)));
