...
public class JobDetailPanel extends DetailPanel
{
    ...
    protected void addHeader()
    {
        ...
        PrettyTimeLabel dueDateLabel =
            new PrettyTimeLabel(i18nManager.getMessage(Messages.JOB_DUEDATE), job.getDuedate(),
    ...
    protected void addJobState()
    {
        ...
        if (job.getExceptionMessage() != null)
        {
            ...
            Label exceptionMessageLabel =
                new Label(i18nManager.getMessage(Messages.JOB_ERROR) + ": " + job.getExceptionMessage());
            ...
            if (job.getProcessDefinitionId() != null)
            {
                ...
                if (jobEntity.getJobHandlerType().equals(TimerSuspendProcessDefinitionHandler.TYPE))
                {
                    addLinkToProcessDefinition(layout,
                        i18nManager.getMessage(Messages.JOB_SUSPEND_PROCESSDEFINITION),
                        false);
                }
                else if (jobEntity.getJobHandlerType().equals(TimerActivateProcessDefinitionHandler.TYPE))
                {
                    addLinkToProcessDefinition(layout,
                        i18nManager.getMessage(Messages.JOB_ACTIVATE_PROCESSDEFINITION),
                        true);
                }
                else
                {
    ...
    protected void addLinkToProcessDefinition(final VerticalLayout verticalLayout, final String labelText,
        final boolean isSuspendedProcessDefinition)
    {
        ...
        showProcessDefinitionLink.addListener(new ClickListener()
        {
            ...
            public void buttonClick(ClickEvent event)
            {
                if (isSuspendedProcessDefinition)
                {
                    ExplorerApp.get()
                        .getViewManager()
                        .showSuspendedProcessDefinitionsPage(job.getProcessDefinitionId());
                }
                else
                {
