...
public interface LoginHandler extends Serializable
{
    ...
    /**
     * Authenticate the current user. Use this to eg. shared autentication, which can be done without the user actually
     * having to provide credentials.
     * 
     * @return The logged in user. Return null, if no user can be logged in automatically. When null is returned, user
     *         will be requested to provide credentials.
     */
    LoggedInUser authenticate(HttpServletRequest request, HttpServletResponse response);
    
    /**
     * Called when the user is logged out, should clear all context related to authorization and authentication for the
