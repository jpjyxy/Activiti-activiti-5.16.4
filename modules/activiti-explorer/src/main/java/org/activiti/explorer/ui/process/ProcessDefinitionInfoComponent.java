    ...
    protected void initSuspensionStateInformation()
    {
        ...
        if (!suspensionStateJobs.isEmpty())
        {
            ...
            for (JobEntity jobEntity : suspensionStateJobs)
            {
                ...
                {
                    Label suspendLabel =
                        new Label(i18nManager.getMessage(Messages.PROCESS_SCHEDULED_SUSPEND,
                ...
                {
                    Label suspendLabel =
                        new Label(i18nManager.getMessage(Messages.PROCESS_SCHEDULED_ACTIVATE,
                            Constants.DEFAULT_TIME_FORMATTER.format(jobEntity.getDuedate())), Label.CONTENT_XHTML);
                    addComponent(suspendLabel);
                }
            }
                ...
                if (!bpmnModel.getFlowLocationMap().isEmpty())
                {
                    ...
                    URL url =
                        new URL(explorerURL.getProtocol(), explorerURL.getHost(), explorerURL.getPort(),
                            explorerURL.getPath().replace("/ui", "") + "diagram-viewer/index.html?processDefinitionId="
