...
public class ProcessInstancePage extends ManagementPage
{
    ...
    protected Table createList()
    {
        ...
        table.addListener(new Property.ValueChangeListener()
        {
            ...
            {
                Item item = table.getItem(event.getProperty().getValue()); // the value of the property is the itemId of
                                                                           // the table entry
                if (item != null)
                {
                    String processInstanceId = (String)item.getItemProperty("id").getValue();
                    setDetailComponent(new AlfrescoProcessInstanceDetailPanel(processInstanceId,
                        ProcessInstancePage.this));
                    
                    // Update URL
                    ExplorerApp.get().setCurrentUriFragment(new UriFragment(
                        ProcessInstanceNavigator.PROCESS_INSTANCE_URL_PART, processInstanceId));
                }
                else
                {
                    // Nothing is selected
                    ...
                    ExplorerApp.get().setCurrentUriFragment(new UriFragment(
                        ProcessInstanceNavigator.PROCESS_INSTANCE_URL_PART));
                }
            }
        });
