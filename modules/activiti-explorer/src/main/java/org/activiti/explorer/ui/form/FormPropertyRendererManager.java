 ...
 * @author Joram Barrez
 * 
 *         Note: NOT configured by @Component, but in Spring XML config, to make it easy for users to extends with
 *         custom renderers.
 */
public class FormPropertyRendererManager implements Serializable
{
    ...
    protected Map<Class<? extends FormType>, FormPropertyRenderer> propertyRenderers =
        new HashMap<Class<? extends FormType>, FormPropertyRenderer>();
    ...
    public FormPropertyRenderer getPropertyRendererForType(FormType formType)
    {
        ...
        if (renderer == null)
        {
            throw new ActivitiIllegalArgumentException("No property renderer found for type: " + formType.getName()
                + ", " + formType.getClass());
        }
        return renderer;
    }
