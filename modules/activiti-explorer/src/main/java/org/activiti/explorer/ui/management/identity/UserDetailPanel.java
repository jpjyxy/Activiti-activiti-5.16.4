...
public class UserDetailPanel extends DetailPanel implements MemberShipChangeListener
{
    ...
    protected void loadUserDetails()
    {
        ...
        // Details
        addUserDetail(detailGrid, i18nManager.getMessage(Messages.USER_ID), new Label(user.getId())); // details are
                                                                                                      // non-editable
        if (!editingDetails)
        {
    ...
    protected void initEditButton(VerticalLayout actionLayout)
    {
        ...
        editButton.addListener(new ClickListener()
        {
            public void buttonClick(ClickEvent event)
            {
                ...
                populateUserDetails(); // the layout will be populated differently since the 'editingDetails' boolean is
                                       // set
            }
        });
    }
        ...
        deleteButton.addListener(new ClickListener()
        {
            public void buttonClick(ClickEvent event)
            {
                ConfirmationDialogPopupWindow confirmPopup =
                    new ConfirmationDialogPopupWindow(
                        i18nManager.getMessage(Messages.USER_CONFIRM_DELETE, user.getId()));
    ...
    protected void initGroups()
    {
        ...
        groupLayout = new HorizontalLayout(); // we wrap the table in a simple layout so we can remove the table easy
                                              // later on
    ...
    protected void initAddGroupsButton(HorizontalLayout groupHeader)
    {
        ...
        addRelatedContentButton.addListener(new ClickListener()
        {
            ...
            public void buttonClick(ClickEvent event)
            {
                final GroupSelectionPopupWindow selectionPopup =
                    new GroupSelectionPopupWindow(identityService, user.getId());
