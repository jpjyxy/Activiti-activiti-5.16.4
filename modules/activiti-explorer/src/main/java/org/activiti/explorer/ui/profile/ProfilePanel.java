...
public class ProfilePanel extends Panel
{
    ...
    protected void initAboutSection()
    {
        ...
        {
            addProfileEntry(aboutLayout, i18nManager.getMessage(Messages.PROFILE_NAME), user.getFirstName() + " "
                + user.getLastName());
        }
        else if (editable)
        {
            ...
            addProfileInputField(aboutLayout,
                i18nManager.getMessage(Messages.PROFILE_FIRST_NAME),
                firstNameField,
                user.getFirstName());
            lastNameField = new TextField();
            addProfileInputField(aboutLayout,
                i18nManager.getMessage(Messages.PROFILE_LAST_NAME),
                lastNameField,
                user.getLastName());
        }
        ...
        else if (editable)
        {
            ...
            addProfileInputField(aboutLayout,
                i18nManager.getMessage(Messages.PROFILE_JOBTITLE),
                jobTitleField,
                jobTitle);
        }
        ...
        else if (editable)
        {
            ...
            addProfileInputField(aboutLayout,
                i18nManager.getMessage(Messages.PROFILE_LOCATION),
                locationField,
                location);
        }
    }
    ...
    protected Button initSaveProfileButton()
    {
        ...
        saveProfileButton.addListener(new ClickListener()
        {
            public void buttonClick(ClickEvent event)
            {
                ...
                identityService.setUserInfo(user.getId(), Constants.USER_INFO_JOB_TITLE, jobTitleField.getValue()
                    .toString());
                if (birthDateField.getValue() != null && !"".equals(birthDateField.getValue().toString()))
                {
                    identityService.setUserInfo(user.getId(), Constants.USER_INFO_BIRTH_DATE, new SimpleDateFormat(
                        Constants.DEFAULT_DATE_FORMAT).format(birthDateField.getValue()));
                }
                identityService.setUserInfo(user.getId(), Constants.USER_INFO_LOCATION, locationField.getValue()
                    .toString());
                identityService.setUserInfo(user.getId(), Constants.USER_INFO_PHONE, phoneField.getValue().toString());
                identityService.setUserInfo(user.getId(), Constants.USER_INFO_TWITTER, twitterField.getValue()
                    .toString());
    ...
    protected void initContactSection()
    {
        ...
        else if (editable)
        {
            ...
            addProfileInputField(contactLayout,
                i18nManager.getMessage(Messages.PROFILE_EMAIL),
                emailField,
                user.getEmail());
        }
        ...
        else if (editable)
        {
            ...
            addProfileInputField(contactLayout,
                i18nManager.getMessage(Messages.PROFILE_TWITTER),
                twitterField,
                twitterName);
        }
