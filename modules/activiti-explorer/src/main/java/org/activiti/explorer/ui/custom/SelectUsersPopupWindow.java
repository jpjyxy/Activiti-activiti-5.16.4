...
/**
 * A popup window that is used to select people. Two possible modes: - multiselect: displays two tables that allow to
 * select users from the left table to the table on the right - non-multiselect: one table where only one user can be
 * chosen from.
 * 
 * {@link SubmitEventListener} can be attached to listen to completion of the selection. The selected user(s) can be
 * retrieved using {@link #getSelectedUserId()} ,{@link #getSelectedUserIds()} and {@link #getSelectedUserRole(String)}.
 * 
 * @author Joram Barrez
 */
public class SelectUsersPopupWindow extends PopupWindow
{
    ...
    public SelectUsersPopupWindow(String title, boolean multiSelect, boolean showRoles,
        ...
        selectedUsersTable.addGeneratedColumn("delete", new ThemeImageColumnGenerator(Images.DELETE,
            new com.vaadin.event.MouseEvents.ClickListener()
            {
                public void click(com.vaadin.event.MouseEvents.ClickEvent event)
                {
                    ...
                    {
                        String userName =
                            (String)selectedUsersTable.getItem(itemId).getItemProperty("userName").getValue();
    ...
    protected void selectUser(String userId, String userName)
    {
        if (!selectedUsersTable.containsId(userId))
        {
            ...
            {
                ComboBox comboBox =
                    new ComboBox(null, Arrays.asList(i18nManager.getMessage(Messages.TASK_ROLE_CONTRIBUTOR),
