...
public class ReportingUtil
{
    ...
    public static ResultSet executeSelectSqlQuery(String sql)
        throws Exception
    {
    ...
    public static void generateTaskDurationReport(String processDefinitionId)
    {
        ...
        String reportDescription =
            "Average task duration report for process definition " + processDefinition.getName() + " ( version "
        ...
        String script =
            "importPackage(java.sql);"
                ...
                + "var reportData = new ReportData();" + "var dataset = reportData.newDataset();"
                + "dataset.type = 'pieChart';" + "dataset.description = '" + reportDescription + "';" + ""
                + "while (result.next()) { " + "  var name = result.getString(1);"
                + "  var val = result.getLong(2) / 1000;" + "  dataset.add(name, val);" + "}" + ""
                + "execution.setVariable('reportData', reportData.toBytes());";
        
        // Generate bpmn model
        WorkflowDefinition workflowDefinition =
            new WorkflowDefinition().name(processDefinition.getName() + " task duration report")
