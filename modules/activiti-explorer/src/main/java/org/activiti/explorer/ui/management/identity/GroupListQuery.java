...
public class GroupListQuery extends AbstractLazyLoadingQuery
{
    ...
    {
        List<Group> groups =
            identityService.createGroupQuery().orderByGroupId().asc().orderByGroupName().asc().listPage(start, count);
    ...
    class GroupListItem extends PropertysetItem implements Comparable<GroupListItem>
    {
        ...
        public GroupListItem(Group group)
        {
            ...
            {
                addItemProperty("name", new ObjectProperty<String>(group.getName() + " (" + group.getName() + ")",
                    String.class));
            }
            else
            {
