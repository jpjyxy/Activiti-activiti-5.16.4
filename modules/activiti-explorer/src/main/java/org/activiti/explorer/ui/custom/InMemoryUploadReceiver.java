...
/**
 * Generic {@link Receiver} for an {@link Upload} component. Stores the bytes in-memory, so be careful to limit the size
 * of the uploads.
 * 
 * @author Joram Barrez
 */
public class InMemoryUploadReceiver implements Receiver, StartedListener, ProgressListener
{
    ...
    public void uploadStarted(StartedEvent event)
    {
        ...
        if (!interrupted)
        { // upload can be interrupted by invalid file size
    ...
    {
        if (contentLength == -1)
        { // Older browsers don't support getting the filesize
    ...
    protected void checkFileSize(long receivedLength)
    {
        ...
        {
            interrupt();
            notificationManager.showWarningNotification(Messages.UPLOAD_FAILED, Messages.UPLOAD_LIMIT, maxFileSize
                / 1024 + "kb");
        }
    }
    ...
    /**
     * By default, all mime types are accepted. By providing a set of mimetypes, the receiver will only work with the
     * given mimetypes
     */
