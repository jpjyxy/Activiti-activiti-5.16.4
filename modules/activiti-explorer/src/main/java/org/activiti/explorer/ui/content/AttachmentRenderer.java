...
/**
 * Representing a class that is capable of rendering related content ({@link Attachment}s) for a certain type in
 * different contexts: overview link, edit/add and detail.
 * 
...
public interface AttachmentRenderer extends Serializable
{
     ...
     * 
     * @param attachment the attachment to get the image for. When null is passed, the image for a new attachment (of
     *            the type this class is capable of rendering) is expected to be returned.
     */
    Resource getImage(Attachment attachment);
    
    /**
     * Get the component that is used in the related content overview list. Use the passed parent for calling back to
     * show detail.
     */
    Component getOverviewComponent(Attachment attachment, RelatedContentComponent parent);
    
    /**
     * Get the component to render when viewing the details of the related content for the type this renderer is
     * responsible for.
     */
