...
public class TaskNavigator implements Navigator
{
    ...
    protected void directToSpecificTaskPage(String category, String taskId, UriFragment uriFragment)
    {
        ...
        else if (CATEGORY_QUEUED.equals(category))
        {
            ...
            boolean isTaskAssignedToGroup =
                taskService.createTaskQuery().taskId(task.getId()).taskCandidateGroup(groupId).count() == 1;
            
            boolean isUserMemberOfGroup =
                identityService.createGroupQuery().groupMember(loggedInUserId).groupId(groupId).count() == 1;
    ...
    protected void showNavigationError(String taskId)
    {
        ...
        ExplorerApp.get()
            ...
            .showErrorNotification(Messages.NAVIGATION_ERROR_NOT_INVOLVED_TITLE, description);
    }
